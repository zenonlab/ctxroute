...
`settings.json` wiring → absolute paths to that folder: `doc-inject.js` (the GATE, 16 frame declarations — setting `frames`, raised from 12 to 16 on 2026-08-12), `ctxroute-reset.js`, `turn-count.js`, `session-inject.js`, `canary-check.js`. ⚠️ `legacy-mcp-inject.js` is NOT wired (relic = differential oracle).
