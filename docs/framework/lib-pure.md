...
⚠️ `refusalNotice(code, state)` + `REFUSAL_CODES` = the DECISION of the dead-authority witness, kept PURE on purpose: written in the spawned shell it would ship measured by nothing. The code list is CLOSED (`ECONNREFUSED`, `ENOENT`) — `EACCES` is deliberately EXCLUDED, since permission denied means someone IS there. Widening it turns a witness into a liar; see `turn-count.md`.
