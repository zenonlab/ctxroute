...
- ⚠️ **TRI-STATE EVERYWHERE**: no corpus, or files read with zero tool call extracted, ⇒ **NOT MEASURED**, exit 2. A `0` would read as "nothing collides" — false good news.
- ⚠️ **THE CORPUS COMES FROM `paths.fileDocsDir()`, never rebuilt here** (21/08/2026): measured against a folder other than the one the ENGINE reads, the measurement answers about nothing — and this tool exists precisely so a decision can be REPLAYED.
