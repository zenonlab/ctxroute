...
⚠️ NEVER delete a case from `lint-corpus.test.js`: the sabotage (fake fleet in tmpdir via `CTXROUTE_FLEET_HOOKS_DIR`/`CTXROUTE_HOME` — the fleet root now comes from `paths.fleetHooksDir()`, its own `CTXROUTE_HOOKS_DIR` retired 21/08/2026) is the ONLY proof that the lint bites. Green on a healthy fleet proves NOTHING. Sabotage verified 15/07: 18/3/2 tests red.
