...
- 🛑 **PRODUCTION RUNS A FROZEN COPY (`Desktop/ctxroute-release`) SINCE 2026-08-24 — NEVER the repo you edit.** `watchOwnCode` makes the daemon exit on its own source change (correct: never serve stale logic), and production used to run from the working directory, so EVERY delivery cost the whole fleet its injection window — 169 deaths in one day, 32 red lines per tool call. 🔴 **AND « the frozen copy makes the CAUSE disappear » WAS FALSE, MEASURED THE SAME DAY**: nothing had written to that copy since it was built and the daemon still killed itself **258 times** — the event that killed it carried `mtime` and `ctime` UNCHANGED, **only `atime` moved**, because libuv subscribes ReadDirectoryChangesW to `FILE_NOTIFY_CHANGE_LAST_ACCESS` and NTFS may defer that write by up to an hour. Reading a file was enough. Freezing removed the WRITES, never the notifications. ✅ The cure is that freshness became an OBSERVATION at the POINT OF USE (`stale-code.md`, entry point `src/hooks/http-daemon.js`), which also closes the symmetric hole all three kernels document — event LOSS. The frozen copy stays for its own reason: a delivery becomes a gesture you choose. **PROVEN, not reasoned**: a file of the working repo edited, daemon alive, HTTP 200. ⚠️ **Hot reload was REFUSED and must never be reopened** — it would MANAGE the death (five permanent proofs to hold) where this removes it. **Delivering = rebuild (`git archive HEAD` + `npm ci`) then restart the task**, a gesture you choose. 🔑 What made it possible: the code no longer decides where the DATA lives — `stateDir`/`docsDir`/`sessionDocsDir` are declarable keys, and the config itself is found at the **OS conventional address** (XDG · `%APPDATA%` · `~/Library/Application Support`), with the launch argument as the explicit override and the reserved test env var still first. That also repairs the ADOPTER, whose config used to live inside the clone and died at the first pull. 🛑 One config, one truth: the repo-local file was REMOVED, not kept as a fallback. ⚠️ A declared address is NORMALISED at the single resolution point — `C:/x` and `C:\x` are one directory and two strings, and a judge comparing them was right to redden. ⚠️ The `command` hooks still point at the working repo, deliberately: they are spawned fresh per event, so stale code cannot reach them; only the long-lived daemon needed this.
