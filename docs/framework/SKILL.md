...
✅ **ONE EXCEPTION, AND IT IS WRITTEN BECAUSE THE RULE FORBADE ITS OWN REPAIR (2026-09-01).** A commit whose ONLY content is the FIX for the current red MAY ship while that red stands. Otherwise the rule is a judge that refuses the progress it demands: the red lives on a platform nobody here owns, the only witness is the CI, and the CI runs **on the public repository alone** — so holding the fix back keeps the badge red for ever and nothing can ever turn it green. ⚠️ **The exception is narrow and it is not a licence**: it covers the repair and nothing riding along with it, the local suite must still be exercised on what CAN be exercised here, and the message must say which red it repairs. 🔑 **AND IT DISAPPEARS THE DAY A PRIVATE REPOSITORY EXISTS** — with a place where CI runs BEFORE publication, the exception has no reason to exist and must be deleted rather than inherited.
