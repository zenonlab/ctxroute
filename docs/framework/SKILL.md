...
⚠️ **LANGUAGE (㉒ REVERSED on 16/08/2026, maintainer decision — replaces "FR for life" of 15/08, executed the same day)**: the WHOLE project is in ENGLISH — identifiers, engine messages (badges/deny/explain/doctor), tests, comments, repo docs. Only the maintainer's personal fleet docs outside the repo may stay French. New code follows: English only.
