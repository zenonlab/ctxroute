...
function clientLane(argv) {
  ...
  if (i < 0) return null;
  const next = argv[i + 1];
  const address = typeof next === 'string' && next !== '' && !next.startsWith('-')
    ? next
    : undefined;
...
function run(data, deps, options) {
  ...
  const engine = deps.run || pretool.run;

  demander(data, { frame: o.frame, frames: o.nbFrames, socketPath: o.socketPath }, (response) => {
    ...
    engine(data, (decision, fullDoc, systemMessage) => {
      emit(output(decision, fullDoc, systemMessage));
    }, { ...o, store: fallback.store, withLock: fallback.withLock });
  });
}

module.exports = { run, clientLane, STATE_ABSENT, WITHOUT_LOCK, LANE_FLAG };
