...
function main() {
  const FLEET = paths.fleetHooksDir();
  ...
  const dest = path.join(FLEET, 'deadline.js');
  const srcContent = fs.readFileSync(SRC, 'utf8');
  const identical = fs.existsSync(dest) && fs.readFileSync(dest, 'utf8') === srcContent;
  if (!identical) {
  ...
  for (const f of fs.readdirSync(FLEET)) {
    ...
    lines.splice(idx, 0, BANNER.replace(/\n/g, eol));
    if (WRITE) fs.writeFileSync(abs, lines.join(eol));
    rapport.armed.push(f);
  }

  console.log(WRITE ? '=== WRITTEN ===' : '=== DRY-RUN (--write to apply) ===');
  console.log(`target         : ${FLEET}`);
  console.log(`deadline.js    : ${rapport.copy ? 'to copy' : 'already identical'}`);
  console.log(`to arm         : ${rapport.armed.length}${rapport.armed.length ? ' → ' + rapport.armed.join(', ') : ''}`);
  ...
  console.log(`⚠️ MANUAL      : ${rapport.manual.length}${rapport.manual.length ? ' → ' + rapport.manual.join(', ') : ''}`);
}
