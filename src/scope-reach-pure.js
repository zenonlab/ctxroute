...
function norm(s) {
  return String(s).toLowerCase().split(ANTISLASH).join('/');
}
...
const NOM = /"name":"([A-Za-z][A-Za-z0-9_-]*)"/;
 ...
 */
function toolNamesFrom(texte) {
  if (typeof texte !== 'string') return [];
  const vus = new Set();
  let i = texte.indexOf(ANCRE);
  while (i !== -1) {
    const trouve = NOM.exec(texte.slice(i, i + FENETRE));
    if (trouve) vus.add(trouve[1]);
    i = texte.indexOf(ANCRE, i + ANCRE.length);
  }
  return [...vus];
}
...
module.exports = { norm, patterns, toolNamesFrom, collides, ANCRE };
