...
function matcherOf(block) {
  return block && typeof block.matcher === 'string' ? block.matcher : null;
}
 ...
 */
function splice(settings, declarations, root) {
  ...
  // Refusing an absent `hooks` section is fail-closed on the likeliest
  // mistake: a path that points at a file which is not the wiring at all.
  if (!hooks || typeof hooks !== 'object' || Array.isArray(hooks)) fail('the target declares no `hooks` section — this is probably not the wiring file you think it is, and a wiring is never created out of nothing here');
  ...
  for (const event of Object.keys(hooks)) {
    ...
    for (const block of blocks) {
      ...
      for (const entry of entries) {
        if (ownedBy(entry, root)) {
          removed += 1;
          const key = `${event}\u0000${matcherOf(block) === null ? '' : matcherOf(block)}`;
      ...
      if (block && Array.isArray(block.hooks)) block.hooks = kept;
    }
    ...
    hooks[event] = blocks.filter((b) => !(b && Array.isArray(b.hooks) && b.hooks.length === 0));
  ...
  for (const [key, group] of byBlock(declarations)) {
    ...
    if (target && Array.isArray(target.block.hooks)) {
      target.block.hooks.splice(target.at, 0, ...entries);
    ...
    const block = matcher === null ? { hooks: entries } : { matcher, hooks: entries };
