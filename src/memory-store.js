...
function createMemoryStore(options) {
  ...
  function loadState(prefix, sessionId) {
    ...
    const v = pur.touch(state, cle(prefix, sessionId));
    return v === undefined ? {} : v;
  }

  function saveState(prefix, sessionId, value) {
    if (estDurable(cle(prefix, sessionId))) {
      ...
      durable.saveState(prefix, sessionId, value);
      return;
    }
    pur.set(state, cle(prefix, sessionId), value);
    ...
    pur.evict(state, MAX_SCOPES, MAX_EPHEMERAL);
    count();
  }
   ...
   */
  function purge(keyPrefix) {
    let n = pur.purge(state, keyPrefix);
    ...
    if (durable !== null) n += durable.purgeByPrefix(keyPrefix);
    if (n > 0) count();
    return n;
  }
   ...
   */
  function count() {
    const t = pur.persistTick(pending, pur.PERSIST_EVERY);
    pending = t.pending;
    if (t.persist) writeSnapshot();
  }
   ...
   */
  function flush() {
    if (!pur.shouldFlush(pending)) return false;
    pending = 0;
    writeSnapshot();
    return true;
  }
