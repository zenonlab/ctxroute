...
function endpoint(options) {
  ...
  const racine = o.root || paths.ROOT;
  const empreinte = fingerprint(racine);
  ...
  if (plateforme === 'win32') return `\\\\.\\pipe\\ctxroute-${empreinte}`;
  ...
  return path.join(o.stateDir || paths.stateDir() || os.tmpdir(), `ctxroute-${empreinte}.sock`);
}
 ...
 */
function leavesFilesystemEntry(plateforme) {
  const p = plateforme || process.platform;
