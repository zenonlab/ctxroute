...
function analyze(etat) {
  ...
  //    `docsFantomes` by difference — the set is structurally empty).
  ...
  const couverts = new Set([...list(e.serveursDocumentes), ...list(e.serveursDeclares)]);
 ...
 *    detection).
 */
function applyFilter(findings, niveau) {
  if (niveau === 'off') return [];
  const seuil = LEVELS.indexOf(LEVELS.includes(niveau) ? niveau : DEFAULT_LEVEL);
  return list(findings).filter((c) => c && LEVELS.indexOf(c.niveau) <= seuil);
}
...
function shouldScream(findings) {
  return list(findings).some((c) => c && c.niveau === 'error');
}
