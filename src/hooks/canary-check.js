...
function readQueue(file) {
  ...
  const size = Math.min(canary.BYTE_WINDOW, st.size);
  if (size === 0) return '';
...
function run(data) {
  ...
  const extract = readQueue(transcript);
  const injections = canary.countInjections(extract);
  ...
  const v = canary.verdict(emissions, injections);
  ...
  fs.writeFileSync(tmp, JSON.stringify({
    ...
    transcript,
    tailleTranscript,
    octetsLus: tailleTranscript < 0 ? -1 : Math.min(canary.BYTE_WINDOW, tailleTranscript),
    scope: scopeId,
  }));
