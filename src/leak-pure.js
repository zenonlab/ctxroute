...
const ALLOWED_IPS = /^(203\.0\.113\.|192\.0\.2\.|198\.51\.100\.|127\.|0\.0\.0\.0)/;
 ...
 * @param {string} user - OS account name (never hard-coded)
 * @param {string} personalFolder - the user's home folder
 * @param {string[]} [extras] - terms from a LOCAL gitignored list
 */
function forbiddenPatterns(user, personalFolder, extras) {
  const patterns = [
    { name: 'real email', re: EMAIL_REEL },
    { name: 'real machine IP (CGNAT/Tailscale)', re: IP_CGNAT },
  ];
  const literals = [];
  if (typeof user === 'string' && user.length >= 3) literals.push(user);
  if (typeof personalFolder === 'string') {
    const d = lastSegment(personalFolder);
    if (d.length >= 3) literals.push(d);
  }
  if (Array.isArray(extras)) {
    for (const t of extras) {
      if (typeof t === 'string' && t.length >= 3) literals.push(t);
    }
  }
  ...
  for (const l of [...new Set(literals)].filter((x) => !GENERIC_ACCOUNTS.has(x.toLowerCase()))) {
...
function scan(text, patterns) {
  ...
  for (const m of patterns) {
    ...
    //    by its own gate.
    if (found && !ALLOWED_IPS.test(found[0])) {
 ...
 * @param {string[]} seen - candidate roots discovered by the caller
 * @param {string[]} derived - roots declared in `derivedFolders`
 * @param {string[]} ignored - roots KNOWINGLY ruled out (`ignoredRoots`)
 * @returns {string[]} the forgotten ones, in discovery order, deduplicated
 */
function forgottenRoots(seen, derived, ignored) {
  ...
  for (const r of Array.isArray(derived) ? derived : []) decided.add(normalizePath(r));
  // Stryker disable next-line ArrayDeclaration
  for (const r of Array.isArray(ignored) ? ignored : []) decided.add(normalizePath(r));
  const out = [];
...
module.exports = {
  forbiddenPatterns, scan, escapeLiteral, lastSegment, GENERIC_ACCOUNTS,
