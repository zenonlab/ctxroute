...
function decide(config, decls, matched, state, turnCount, owners, toolName) {
  ...
  for (const doc of Object.keys(prev)) {
    ...
    if (!matchedSet.has(doc) && entry && modeForDoc(config, decls[doc], src(doc)) === 'smart'
      ...
      //    LITERAL here re-declares the WHOLE entry, so every field absent from the
      //    literal is DROPPED, silently, with nothing red anywhere.
      //    THE DEFECT THIS FIXES: `denied` was lost. That flag IS the anti-loop — this
      //    file states in bold "a block is NEVER followed by a block" and anchors the
      //    guarantee in it. Any tool call not matching the document erased it, so the
      //    gesture the agent redoes was refused a SECOND time with no successful pass in
      //    between. MEASURED against the independent model: 28 of 1,024 decisions
      //    diverged, ALL in the same direction (engine `deny`, model `allow`), and the
      //    three conditions (`enforce` + `smart` + driftUnit `tool`) are met in production.
      //    `turn` was lost by the same gesture; the spread restores it too.
      // ⚠️ THE SPREAD IS OBLIGATORY, and it is not a style choice: it is what makes the
      //    entry's shape OPEN. A field added tomorrow (another memory of the alternation,
      //    another anchor) survives a foreign action BY CONSTRUCTION, instead of being
      //    born inert the way `denied` was. A literal only ever knows the fields that
      //    existed the day it was written.
      // 🛑 `seen` is NOT restated: it is `true` in every entry decide() writes, so
      //    spelling it out again would be an equivalent mutant AND would re-introduce the
      //    very habit this comment forbids.
