...
function sweep(options) {
  ...
  for (const itemName of itemNames) {
    let st;
    try {
      st = fs.statSync(path.join(dir, itemName));
    } catch {
    ...
    entries.push({ name: itemName, mtimeMs: st.mtimeMs });
  }
  ...
  for (const itemName of plan.remove) {
    try {
      fs.unlinkSync(path.join(dir, itemName));
      result.removed.push(itemName);
    } catch {
