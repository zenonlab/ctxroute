...
function extractFilePaths(toolName, toolInput, profile) {
...
//    collected over 329 transcripts): REAL max depth = 11 (mailer), max
...
function textValues(value, depth, state, key, decision) {
  const acc = state || { chunks: [], keys: [], size: 0, truncated: null };
  ...
  if (typeof value === 'string') {
    ...
    if ((depth || 0) >= MAX_DEPTH) acc.truncated = acc.truncated || 'depth';
      ...
      if (!DEFAULT_PROFILE.contentKeys.includes(sousCle) || decision?.guard(sousCle)) {
...
function keyDecision(keysDecl, axis) {
  ...
  for (const k of decl) {
    const s = String(k);
    if (s.startsWith(KEY_REMOVE)) banned.push(s.slice(KEY_REMOVE.length));
    else additions.push(s);
  }
  const adjusted = banned.length > 0;
  return {
    ...
    guard: adjusted
      ? (key) => additions.includes(key) || (!banned.includes(key) && !DEFAULT_PROFILE.contentKeys.includes(key))
      : (key) => additions.includes(key),
    adjusted,
    banned,
    additions,
    decl,
...
function composerKeys(entree, fallbackValue) {
  ...
  if (!dE) return fallbackValue;        // an entry naming nothing narrows nothing: the category stands.
  if (!dE.adjusted) return entree; // REPLACE: "only these", category included.
  const dD = keyDecision(fallbackValue, null);
  if (!dD) return entree;
  if (dD.adjusted) return dD.decl.concat(dE.decl);
  return dD.additions.filter((k) => !dE.banned.includes(k)).concat(dE.additions);
}
...
function composerKeysParAxe(entree, fallbackValue) {
  if (entree === undefined) return fallbackValue;
  if (fallbackValue === undefined) return entree;
  ...
  if (plat(entree) && plat(fallbackValue)) return composerKeys(entree, fallbackValue);
  const out = {};
  for (const axe of AXES_KEYS()) {
    const e = Array.isArray(entree) ? entree : (entree && entree[axe]);
    const d = Array.isArray(fallbackValue) ? fallbackValue : (fallbackValue && fallbackValue[axe]);
...
function inheritFilters(entree, sourceDefaults) {
  const d = (sourceDefaults && !Array.isArray(sourceDefaults)) ? sourceDefaults : {};
  ...
  const choose = (a, b) => ((Array.isArray(a) && a.length) ? a : ((Array.isArray(b) && b.length) ? b : undefined));
  const out = { keys: composerKeysParAxe(e.keys, d.keys) };
  const sc = choose(e.scope, d.scope);
  const ex = choose(e.exclude, d.exclude);
  if (sc) out.scope = sc;
...
function triggerKeys(keysDecl, sourceDefaults) {
  const d = keyDecision(keysDecl, 'match');
  if (!d) return sourceDefaults;
  ...
  if (!d.adjusted) return d.additions;
  return sourceDefaults.filter((k) => !d.banned.includes(k)).concat(d.additions);
}
...
function shouldSkip(rule, context, toolInput) {
  ...
  const groups = scopeGroups(rule.scope);
  // AND between the GROUPS, OR within a group. Flat form = ONE group = the old OR.
  if (groups && !groups.every((g) => g.some((s) => values.some((v) => v.includes(norm(s)))))) return true;
  return false;
}
...
function matchingDocs(rules, payload) {
  ...
  const commands = keyValues(toolInput, DEFAULT_PROFILE.commandKeys, MAX_DEPTH);
  ...
  if (commands.some((c) => /^\s*git\s+/.test(c))) return [];
  ...
  for (const rule of rules) {
    ...
    const mordants = [];
    for (const fp of pathsR) if (norm(fp).includes(normPattern)) mordants.push(fp);
    for (const command of commandsR) {
      if (norm(command).includes(normPattern)) mordants.push(command);
    }
    for (const obs of DERIVED_OBSERVABLES) {
      ...
      const observableKeys = rule.keys ? triggerKeys(rule.keys, [obs.name]) : null;
      if (observableKeys && !observableKeys.includes(obs.name)) continue;
      ...
      for (const command of commands) {
        for (const candidate of obs.derive(command)) {
          if (norm(candidate).includes(normPattern)) mordants.push(candidate);
        }
      }
    }
