...
const CANARY = path.join(__dirname, '..', 'src', 'hooks', 'canary-check.js');
...
if (idxH !== -1 && process.argv[idxH + 1]) {
  ...
  for (const c of r.required) console.log(`  ${c.present ? '✓' : '✗ REQUIRED MISSING'} ${c.capability} — ${c.role}`);
...
function probe() {
  ...
  try {
    ...
    const canaryPasses = (injects) => {
      const health = path.join(stateDir, 'canary.json');
      try { fs.rmSync(health, { force: true }); } catch { /* first pass */ }
      ...
      const r9 = spawnSync(process.execPath, [CANARY], {
        input: JSON.stringify({
      ...
      let verdict = null;
      try { verdict = JSON.parse(fs.readFileSync(health, 'utf8')).verdict; } catch { /* stays null */ }
      // MUTE BY CONTRACT: on UserPromptSubmit, stdout is injected into the
      // context and an exit ≠ 0 BLOCKS the user's prompt.
      return { verdict, mute: r9.status === 0 && (r9.stdout || '').trim() === '' };
    };
    const cMort = canaryPasses(false);
    const cVivant = canaryPasses(true);
    check('the CANARY detects a DEAD channel (30 calls, 0 injection landed) and stays MUTE',
      cMort.verdict === 'dead' && cMort.mute,
    ...
    check('the CANARY says `alive` (alive) as soon as ONE injection lands (it does not scream at healthy state)',
      cVivant.verdict === 'alive' && cVivant.mute,
