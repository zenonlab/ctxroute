...
function write(target, declarations, root, shape) {
  let raw;
  ...
  const result = splice(current, declarations, root, shape);
...
function main() {
  ...
  // The harness DOCUMENT: our declarations shaped and serialized the way the
  // declared harness writes a wiring, ready to be pasted by whoever adopts the
  // framework. Distinct from `--out`, which emits the NEUTRAL declaration list
  // the drift gate compares — one is for a human, the other for a judge, and
  // merging them would make each worse at its job.
  ...
  const modes = [out, target, document].filter(Boolean).length;
  if (modes === 0) refuse('one of --out <file> (the neutral declaration list), --document <file> (the harness-shaped wiring, in its own format) or --write <settings.json> (splice into a real wiring) is required');
  if (modes > 1) refuse('--out, --document and --write are exclusive: emitting a list, emitting a document and modifying a wiring are three different acts, and they are typed separately on purpose');
  ...
  // ── THE HARNESS DIALECT, READ AFTER THE PLAN ───────────────────────
  // AFTER, on purpose: a manifest that declares NO consumer is a more
  // fundamental fault than a malformed layout, and the reader must be told the
  // first one first. Every mode below needs it — including `--out`, so a
  // manifest whose dialect is unusable is red on the judge's own path and not
  // only the day somebody generates a real document.
  const d = dialect(manifest);

  if (target) {
    // 🛑 WHETHER WE MAY EDIT THE OPERATOR'S ONLY COPY IS DECLARED, NEVER
    //    ASSUMED. A harness whose wiring this tool cannot rewrite losslessly
    //    gets the FRAGMENT mode and its written reason — a half-written wiring
    //    is worse than none, and "it probably works" is not a proof.
    const obstacle = spliceObstacle(d);
    if (d.write.mode !== 'splice') {
      refuse(`${JSON.stringify(d.name)} declares \`write.mode: ${JSON.stringify(d.write.mode)}\`, so --write is REFUSED for this manifest.\n`
        + `  Declared reason: ${d.write.reason}\n`
        + '  Emit the block with --document <file> and paste it, which is an honest delivery: a half-written wiring is worse than none.');
    }
    if (obstacle !== null) {
      refuse(`${JSON.stringify(d.name)} declares \`write.mode: "splice"\` and its wiring cannot be spliced: ${obstacle}.\n`
        + '  Declare `write.mode: "fragment"` with its reason, and emit the block with --document <file>.');
    }
    const done = write(target, declarations, root, {
      rootKey: d.layout.container[0],
      entriesKey: d.layout.entriesKey,
      matcherKey: d.layout.matcherKey,
    });
  ...
  if (document) {
    const { document: doc, skipped, skippedEvents } = render(declarations, d);
    fs.writeFileSync(document, serialize(doc, d.format), 'utf8');
    // 🛑 A SKIPPED PATH IS NAMED, ALWAYS — `--quiet` silences the count, never
    //    an absence. A hook missing from a wiring looks exactly like one that
    //    works, and this is the only moment anybody can be told.
    if (skipped.length > 0) {
      process.stderr.write(`wiring-generate: ${skipped.length} declaration(s) SKIPPED — ${JSON.stringify(d.name)} has no ${skippedEvents.join(', ')} event:\n`
        + skipped.map((x) => `    ${x.event}: ${x.command || x.url}\n`).join('')
        + '  Those paths are not wired on this harness. That is a harness capability gap, stated here; it is never bodged onto a neighbouring event.\n');
    }
    if (process.argv.includes('--quiet')) return;
    process.stdout.write(`${declarations.length - skipped.length} declaration(s) as ${d.format} for ${d.name} -> ${posix(document)}\n`);
    return;
  }
