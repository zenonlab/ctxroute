...
function diagnose(docId, text, payload) {
  ...
  if ('tool' in fm) {
    ...
    if (toolSource.matchingDocs([{ doc: docId, fm: { ...fm, scope: undefined } }], payload).length > 0) {
      ...
      if (t) d.piege = `⚠️ Payload TRUNCATED (${t}) before the scope was evaluated — bounds ${fileSource.MAX_DEPTH} depth levels / ${fileSource.MAX_SIZE} characters. The term you are looking for may live in the unread part.`;
      return d;
    }
  ...
  if (rules.length === 0) {
    ...
    if (inertes.length > 0) {
      ...
      return { ...base, injects: false, motif: `INERT TRIGGER HERE: \`${inertes.join('`/`')}\` is consumed by NO source of the file corpus`, piege: 'An MCP doc is triggered by its PATH (docs/mcp/{server}.md), never by a frontmatter key. Move the file. Target REFACTOR-PLAN §A.' };
    }
...
function render(a, res, diag, config) {
  ...
  const ecartes = res.acc.matched.filter((d) => !res.inject.includes(d) && !filtered.includes(d));
  if (ecartes.length) {
    L.push('');
    L.push('MATCHED BUT DISCARDED BY THE CADENCE — ' + ecartes.length);
...
function main() {
  ...
  if (a.json) {
    console.log(JSON.stringify({ payload: { toolName: a.toolName, toolInput: a.toolInput }, inject: res.inject, decision: res.decision, matched: res.acc.matched, diagnostic: diag ? { doc: diag.doc, injects: diag.injects, motif: diag.motif, piege: diag.piege || null, detail: diag.detail || null } : null }, null, 2));
  } else {
