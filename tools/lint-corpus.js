 ...
 */
function declarationOf(absolutePath, relativePath, rulesPerDoc) {
  let text = '';
  try {
  ...
  const rules = rulesPerDoc.get(relativePath);
 ...
 */
function hasSourceTag(absolutePath) {
  let text = '';
  try {
    text = fs.readFileSync(absolutePath, 'utf8');
  } catch {
...
function collectDocs() {
  ...
  for (const r of rules) {
    ...
    if (!rulesPerDoc.has(r.doc)) rulesPerDoc.set(r.doc, []);
    rulesPerDoc.get(r.doc).push(r);
  }

  const onDisk = listMdFiles(DOCS).map((rel) => `docs/${rel}`);
  const docs = onDisk.map((rel) => ({
    filePath: rel,
    declaration: declarationOf(path.join(HOOKS, rel), rel, rulesPerDoc),
  ...
  return {
    state: {
      ...
      declaredServers: Array.isArray(config.filterList) ? config.filterList : [],
    },
    level: (config.lint && config.lint.level) || DEFAULT_LEVEL,
