...
// vitest â€” config DEDICATED TO STRYKER (stryker.conf.json â†’ vitest.configFile).
// âš ï¸ Includes ONLY the DETERMINISTIC suites covering the mutated modules.
//    - NEVER the property tests (slow, non-deterministic: one flaky run
//      per mutant = a score that lies) â€” their invariant MUST have its
...
// âš ï¸ New mutated pure module â‡’ its deterministic suite is added HERE (and the
...
export default defineConfig({
  test: {
    ...
    'test/corpus-cache.test.js',
      ...
      'test/canary.test.js',
      // âš ï¸ The stale-code verdict (2026-08-24): deterministic, hermetic, and the
      ...
      // âš ï¸ English-only COMMIT gate: deterministic suite covering src/commit-msg-lang.js.
      'test/commit-msg-lang.test.js',
      // âš ï¸ Anti-leak COMMIT gate (2026-08-27): deterministic suite covering
      ...
      // âš ï¸ English-only IDENTIFIER gate. `foreign-identifier-gate.test.js` spawns `git`
      ...
      //    `mutation.yml` â€” measured 2026-08-25: the module was mutated and measured by
      ...
      // would sit at NoCoverage while the module looks measured â€” the exact hole
      ...
      // protocol-routes suite above â€” absent here, its mutants sit at
      // NoCoverage while the module looks measured.
      ...
      //    Absent from this list its cases exist but Stryker never runs them â‡’ the
      //    operator's branches sit at NoCoverage while the fleet suite is green â€”
      ...
      // âš ï¸ The judge of `language-spec.js` is the EXHAUSTIVE differential: a mutant
      //    of the spec must be killed by the confrontation with the engine. It is
      //    deterministic (total enumeration, zero randomness) â€” it belongs here.
      ...
      'test/gate.test.js',
      // âš ï¸ The cadence differential exercises `gate.js` â€” WHICH IS MUTATED â€” over 11,346
      ...
      // âš ï¸ DETERMINISTIC half only â€” the LAWS of the scanner live in the same file
      ...
      // âš ï¸ The DETERMINISTIC half of the disk-writer gate. `disk-writers-gate.test.js`
      //    spawns `git` and `ast-grep`, so it never enters this runner; absent
      //    this line the verdict would be MUTATED and measured by NOTHING â€”
      //    exactly the "misleading massacre" the header above warns about.
      'test/disk-writers-pure.test.js',
      // âš ï¸ The DECISION half of the fleet's DEAD-MAN SWITCH. `doctor.test.js` spawns the real tool
      //    and never enters this runner; absent this line the split-brain, frame-coordinate and
      //    lane-coherence verdicts would be MUTATED and measured by NOTHING â€” a misleading
      //    massacre on the one judge that decides whether the framework is alive at all.
      'test/doctor-wiring-pure.test.js',
      // âš ï¸ The DETERMINISTIC half of the quadratic gate. `quadratic-gate.test.js`
      ...
      //    be mutated and measured by NOTHING â€” a misleading massacre.
      'test/quadratic-budget-pure.test.js',
      // âš ï¸ The DETERMINISTIC half of the rendezvous gate (2026-08-25).
      ...
      //    `rendezvous-budget-pure.js` would be mutated and measured by NOTHING â€”
      //    the exact hollow green `mutation-workflow-gate` was extended to catch.
      'test/rendezvous-budget-pure.test.js',
      // âš ï¸ The DETERMINISTIC half of the state-entry-rebuild gate.
      ...
      //    measured by NOTHING â€” a misleading massacre.
      'test/state-entry-rebuild-pure.test.js',
      // âš ï¸ `memory-store.js` is the I/O shell and is NOT mutated; its DECISIONS
      //    live in `memory-store-pure.js`, which is â€” and this suite is what
      //    kills those mutants. Absent from this list, the LRU and the ceiling
      //    would be measured by nothing.
      'test/memory-store.test.js',
      // âš ï¸ `state-eviction.js` is the I/O shell (readdir/unlink) and is NOT mutated; the rule that
      //    decides WHAT disappears lives in `state-eviction-pure.js`, which is â€” and this is the
      ...
      // âš ï¸ The DETERMINISTIC half of `state-crdt-pure.js` â€” the lattice laws AND the
      ...
      //    NOTHING â€” a structure whose whole claim is "these writes commute",
      //    believed proven.
      'test/state-crdt-pure.test.js',
      // âš ï¸ The rendezvous address: a PURE function, deterministic, spawn-free.
      ...
      // âš ï¸ The DRIFT GATE spawns and skips without a machine wiring, so it carries NO
      //    mutant. Only this in-process suite can kill them â€” absent, `wiring-plan.js`
      //    would be mutated and measured by NOTHING.
      'test/wiring-plan.test.js',
      // âš ï¸ The SPLICE half of the same module â€” the decision that removes
      ...
      //    fleet suite is green â€” a decision that DELETES configuration,
      //    believed mutated and measured by NOTHING.
      'test/wiring-plan-splice.test.js',
      // âš ï¸ The HARNESS DIALECT of a wiring â€” the decision that turns the neutral
      ...
      //    measured by NOTHING â€” a renderer that silently drops a field, believed
      //    covered.
      'test/wiring-dialect.test.js',
      // âš ï¸ `lifecycle-log.js` is the I/O shell (stat/rename/append) and is NOT
      //    mutated; the rules that decide WHEN the journal turns over and WHAT a
      //    record looks like live in `lifecycle-log-pure.js`, which is â€” and this
      ...
      //    `model-twin-pure.js` would be mutated and measured by NOTHING â€” a misleading massacre
      //    on the judge whose entire job is to keep the OTHER judges honest.
      'test/model-twin-pure.test.js',
      // âš ï¸ The DETERMINISTIC half of the declarable addresses (state, MCP docs,
      ...
      //    Absent from this list, an inverted order â€” the config beating
    ...
    //    INSTRUMENTATION with 3 concurrent runners â€” measured 16/08/2026:
    //    dry run > 5,000 ms â‡’ vitest's default wall hit, ENTIRE run aborted,
