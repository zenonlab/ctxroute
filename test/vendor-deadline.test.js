...
const present = fs.existsSync(FLEET);
...
function cloneFleet(withDocs) {
  ...
  if (withDocs && fs.existsSync(path.join(FLEET, 'docs'))) {
    fs.cpSync(path.join(FLEET, 'docs'), path.join(dir, 'docs'), { recursive: true });
  }
  return dir;
}
...
function hookSurvives(file, timeoutMs) {
...
test('DRY-RUN modifies NO file', { skip, timeout: 300000 }, () => {
  const dir = stripBanner(cloneFleet());
...
test('ANTI-REGRESSION — the 9 suites of the fleet stay GREEN after the patch', { skip, timeout: 300000 }, () => {
  ...
  const before = stripBanner(cloneFleet(true));
  const after = stripBanner(cloneFleet(true));
  patcher(after, true);

  const suites = fs.readdirSync(FLEET).filter((f) => f.endsWith('.test.js'));
  assert.ok(suites.length > 0, 'no suite found in the fleet — blind test');

  const regressions = [];
  for (const s of suites) {
    if (!fs.existsSync(path.join(before, s))) continue;
    const a = spawnSync(process.execPath, ['--test', s], { cwd: before, encoding: 'utf8', timeout: 60000 });
...
test('BEFORE the patch — a hook whose stdin is never closed DOES NOT DIE (the real bug)', { skip, timeout: 300000 }, async () => {
  ...
  const dir = stripBanner(cloneFleet());
  const r = await hookSurvives(path.join(dir, 'statusline.js'), 2500);
...
test('AFTER the patch — EVERY hook of the fleet dies on its own', { skip, timeout: 300000 }, async () => {
  ...
  for (const f of hooks) {
    ...
    if (!r.mort) survivors.push(f);
  }

  assert.deepStrictEqual(survivors, [], `ZOMBIE(S) despite the patch: ${survivors.join(', ')}`);
});
