...
test('⚙️ NO HEAVY suite in the fast lane (derived from the CONTENT, never from a list)', () => {
  const faults = [];
  for (const f of byName.unit.include) {
...
test('⚙️ `isolate: false` ONLY on the fast lane — its condition is PROVEN, not hoped for', () => {
  ...
  assert.strictEqual(byName.unit.isolate, false, 'the fast lane loses its accelerator');
  assert.notStrictEqual(byName.integration.isolate, false, 'ISOLATION DISABLED on suites with side effects: guaranteed false reds');
});
...
test('⚙️ THE HISTORICAL EXCLUSIONS stay out of BOTH lanes', () => {
  ...
  const tout = [...byName.unit.include, ...byName.integration.include];
