...
test('TOTAL: absurd inputs ⇒ never a throw, never an accusation', () => {
  ...
  for (const wrong of [undefined, null, NaN, -3, 1.5, '30', {}, []]) {
    assert.equal(verdict(wrong, 0), 'undecidable', 'emissions=' + String(wrong));
    assert.equal(verdict(EMISSIONS_THRESHOLD, wrong), 'dead', 'injections=' + String(wrong));
  }
});
...
test('COUNTING: only OUR marks count — harness noise is ignored', () => {
  ...
  const s = harnessNoise() + injection() + harnessNoise() + harnessNoise();
  assert.equal(countInjections(s), 1);
  assert.equal(countInjections(harnessNoise().repeat(50)), 0);
});
...
test('COUNTING: non-string input ⇒ zero, never a throw', () => {
  for (const wrong of [undefined, null, 42, {}, []]) {
    assert.equal(countInjections(wrong), 0);
  }
});
