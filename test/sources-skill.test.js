...
test('skillRules: 1 rule per perimeter pattern, prefixed doc', () => {
  const rules = skillRules({ skills: { 'acme-infra': { match: ['platform-mcp', 'acme-infra'] } } });
  expect(rules).toEqual([
    { pattern: 'platform-mcp', doc: 'skill/acme-infra' },
...
test('skillRules: match absent/non-array = 0 rules; skills absent = []', () => {
  ...
  expect(skillRules({ skills: { a: { match: 'platform-mcp' } } })).toEqual([]);
...
test('matchingSkills: perimeter touched -> skill triggered', () => {
  const config = { skills: { 'acme-infra': { match: ['platform-mcp'] } } };
  const hit = matchingSkills(config, { toolName: 'Read', toolInput: { file_path: 'C:/Users/dev/Desktop/platform-mcp/server.js' } });
...
test('matchingSkills: 2 patterns of the SAME skill match -> only 1 pointer (dedup)', () => {
  const config = { skills: { a: { match: ['infra', 'platform-mcp'] } } };
  const hit = matchingSkills(config, { toolName: 'Read', toolInput: { file_path: 'x/platform-mcp/y.js' } });
...
test('matchingSkills: triggered by the SERVER even without touching a file', () => {
  const config = { skills: { 'acme-infra': { match: ['platform-mcp'], servers: ['infra'] } } };
...
test('matchingSkills: file AND server match the same skill -> ONLY 1 pointer (dedup)', () => {
  const config = { skills: { a: { match: ['platform-mcp'], servers: ['infra'] } } };
  // an mcp__infra__ call one of whose params also contains "platform-mcp": the 2 dimensions match
  const hit = matchingSkills(config, { toolName: 'mcp__infra__infra_call', toolInput: { path: 'x/platform-mcp/y' } });
  expect(hit).toEqual([{ doc: 'skill/a' }]);
});
...
test('serverMatches TOOL grain: "srv/tool" only matches THAT tool of the server', () => {
  const config = { skills: { a: { match: ['x'], servers: ['mailer/send_mail'] } } };
  expect(serverMatches(config, { toolName: 'mcp__mailer__send_mail', toolInput: {} })).toEqual([{ doc: 'skill/a' }]);
  expect(serverMatches(config, { toolName: 'mcp__mailer__list_events', toolInput: {} })).toEqual([]);
...
test('serverMatches: the WHOLE-server grain keeps matching all its tools', () => {
  const config = { skills: { a: { match: ['x'], servers: ['mailer'] } } };
  expect(serverMatches(config, { toolName: 'mcp__mailer__send_mail', toolInput: {} })).toEqual([{ doc: 'skill/a' }]);
  expect(serverMatches(config, { toolName: 'mcp__mailer__nimporte', toolInput: {} })).toEqual([{ doc: 'skill/a' }]);
});
...
test('serverMatches: without a resolved sub-tool, a pathological "srv/null" entry NEVER matches', () => {
  ...
  // (never the string 'mailer/null').
  const config = { skills: { a: { match: ['x'], servers: ['mailer/null'] } } };
  expect(serverMatches(config, { toolName: 'mcp__mailer__send_mail', toolInput: {} })).toEqual([]);
});
...
test('skillRules/rules: one scope PER pattern — heterogeneous without duplicating the skill', () => {
  ...
  // client-sites: NO scope, passes everywhere.
  expect(matchingSkills(config, { toolName: 'Read', toolInput: { file_path: 'C:/x/client-sites/y.md' } })).toEqual([{ doc: 'skill/a' }]);
});
...
test('㊴ toolMatches: `tool` triggers the skill (EXACT name)', () => {
  const config = { skills: { 'client-x': { tool: ['mcp__mailer__mailer_call'], scope: ['acme'] } } };
  const payload = { toolName: 'mcp__mailer__mailer_call', toolInput: { args: { to: 'acme@example.com' } } };
...
test('㊴ toolMatches: NEGATIVE — scope not satisfied ⇒ SILENCE', () => {
  ...
  const config = { skills: { 'client-x': { tool: ['mcp__mailer__mailer_call'], scope: ['acme'] } } };
  const payload = { toolName: 'mcp__mailer__mailer_call', toolInput: { args: { to: 'other@example.com' } } };
...
test('㊴ serverMatches: `scope` now FILTERS (before: ALL OR NOTHING)', () => {
  const config = { skills: { 'client-x': { servers: ['mailer'], scope: ['acme'] } } };
  const ok = { toolName: 'mcp__mailer__mailer_call', toolInput: { args: { to: 'acme@example.com' } } };
  const ko = { toolName: 'mcp__mailer__mailer_call', toolInput: { args: { to: 'other@example.com' } } };
...
test('㊴ serverMatches: `exclude` bears on the TOOL NAME (parity with the tool source)', () => {
  const config = { skills: { s: { servers: ['mailer'], exclude: ['send_gmail'] } } };
  const blocked = { toolName: 'mcp__mailer__send_gmail_message', toolInput: {} };
  const passe = { toolName: 'mcp__mailer__list_labels', toolInput: {} };
  expect(serverMatches(config, blocked)).toEqual([]);
