...
//    fleet docs taught the OPPOSITE of the code (`pw-mcp-child-guard.md`
//    mandated the `stdio:'ignore'` that WAS the defect to fix ·
//    `pw-mcp-transports.md` claimed "not compliant with the 404" two hours
//    after compliance was reached · `pw-mcp-concierge.md` described an
...
function localise(rel) {
  for (const base of [REPO, path.join(REPO, 'src'), path.join(REPO, 'src', 'sources'), path.join(REPO, 'src', 'hooks'), path.join(REPO, 'tools'), PARC]) {
  ...
  for (const doc of docsDuMiroir()) {
    ...
    for (const rel of fichiersCites(texte)) {
      ...
      if (base === null) {
        ...
        morts.push(doc + ' cites `' + rel + '` — NOT FOUND (repo, sources/, fleet)');
      } else verifies++;
    }
  }
