...
const segments = (n, size) => Array.from({ length: n }, (_, k) => seg('d' + k, size));
...
test('the header ANNOUNCES the marker BEFORE the content (survives a truncation)', () => {
  ...
  const r = plan([seg('a', 600)], 1000);
  const announcementPos = r.text.indexOf('###END:' + r.marker + '###');
  const contentPos = r.text.indexOf('xxx');
  assert.ok(announcementPos < contentPos, 'the marker must be announced before the content');
});

test('MIXED: what does not fit is DEFERRED, counted and NAMED', () => {
  const list = segments(6, 300);
...
test('invalid budget → FRAMEWORK default (cascade authority ①, never a blockage)', () => {
  for (const wrong of [undefined, null, 0, -1, NaN, Infinity, 'x']) {
    const r = plan([seg('a', 100)], wrong);
    assert.deepStrictEqual(r.emitted, ['a'], 'budget ' + String(wrong));
  }
...
test('DEFAULT_BUDGET stays below the lowest measured harness threshold (10 000)', () => {
  ...
  // ⚠️ THE EXACT VALUE IS ASSERTED, and the literal below is COPIED from
  //    src/budget.js — never read back from the module, which would prove x === x.
  //    Without it, a mutant moving 8000 anywhere inside ]0, 10000[ SURVIVES every
  //    other cell here: the bounds alone leave the margin itself unguarded, and the
  //    margin IS the contract this file exists to hold.
  assert.strictEqual(DEFAULT_BUDGET, 8000, 'the conservative floor moved');
});
...
test('ANNOUNCEMENT: EXACT text, list prefix and line separator', () => {
  const r = plan(segments(6, 300), 1200);
...
test('SEQUENCE: each non-empty frame carries its number k/N and the COMMON marker', () => {
  ...
  const p = planFrames(segments(8, 400), 1200, 4).filter((x) => x.text !== '');
...
test('BOUND: no frame exceeds the budget', () => {
  for (const p of planFrames(segments(10, 500), 1500, 5)) {
...
test('GIANT SEGMENT: CHUNKED and DELIVERED — undeliverability is impossible', () => {
  ...
  const reassembled = p.flatMap((x) => x.text.split('⟦').slice(1))
    .map((m) => m.slice(m.indexOf('⟧\n') + 2))
    .join('')
    .replace(/\n\n---\n\n|\n\n###END:[0-9a-f]{8}###/g, '');
  assert.strictEqual(reassembled.replace(/[^x]/g, '').length, 5000, 'all 5000 characters arrived');
});
...
test('DETERMINISM: two independent computations return the SAME split', () => {
  ...
  const a = planFrames(segments(9, 450), 1300, 4);
  const b = planFrames(segments(9, 450), 1300, 4);
  assert.deepStrictEqual(a, b);
});

test('PRIORITY ORDER preserved: the best ranked is in the first frame', () => {
  const p = planFrames(segments(8, 400), 1200, 4);
...
test('CHUNKING BOUNDARY: at the EXACT size the doc stays WHOLE, beyond it it is split', () => {
  ...
  const tight = planFrames(trois400(), 715, 6);
  const emitted = tight.flatMap((p) => p.emitted);
...
test('FILLING BOUNDARY: a frame FILLED TO THE BRIM is valid', () => {
  ...
  const tight = planFrames(quatre400(), 1122, 2);
  assert.deepStrictEqual(tight.map((p) => p.emitted), [['a'], ['b']]);
  assert.deepStrictEqual(tight[1].deferred.map((d) => d.id), ['c', 'd']);
});
...
test('REMAINDER = a DELAY, never "too big" NOR a CONFIG error', () => {
  ...
  const enough = planFrames(quatre400(), 1122, 8);
  assert.deepStrictEqual(enough.flatMap((x) => x.deferred), [], 'enough frames ⇒ everything goes through');
});
...
const H_CHUNK = '⟦ A — CHUNK 999/999 : reassemble the 999 chunks in order before reading ⟧\n'.length;
const CAP5 = H_CHUNK + 5; // ⇒ `usable` = 5 characters of content per slice
...
test('MARKER: sensitive to the CONTENT and to the NUMBER of frames', () => {
  ...
  const otherText = [{ id: 'a', text: 'y'.repeat(400), label: 'a.md' }, seg('b', 400), seg('c', 400)];
  assert.notStrictEqual(planFrames(otherText, 716, 3)[0].marker, m3, 'the content too');
});
...
test('FRAMES — invalid nbFrames ⇒ SINGLE frame (cascade, never a lopsided split)', () => {
  ...
  const l = () => segments(6, 300);
  for (const wrong of [undefined, null, 0, 1, -3, 2.5, NaN, 'x']) {
    const p = planFrames(l(), 1200, wrong);
    assert.strictEqual(p.length, 1, 'nbFrames=' + String(wrong));
...
test('FRAMES — n=1: PERFECT parity if everything fits, CHUNKING as soon as there is surplus', () => {
  ...
  const fits = () => segments(2, 300);
  assert.deepStrictEqual(planFrames(fits(), 8000, 1), [plan(fits(), 8000)]);

  // As soon as there is surplus, we chunk and we emit — instead of giving up.
  const overflows = () => segments(6, 300);
  const p = planFrames(overflows(), 1200, 1);
...
test('frameCapacity: PHYSICAL bound derived from the real header, never a constant', () => {
  ...
  for (const wrong of [undefined, null, 1, 0, -4, 2.5, 'x']) {
    assert.strictEqual(frameCapacity(8000, wrong), 7684, 'absurd nbFrames ⇒ minimal width (2), never a throw');
  }
});
...
test('frameCapacity: at the EXACT capacity the doc stays whole, one character more ⇒ 2 chunks', () => {
  ...
  const emittedAt = trop.flatMap((p) => p.emitted).filter((id) => id.startsWith('a'));
  assert.deepStrictEqual(emittedAt, ['a#1/2', 'a#2/2'], 'one character more ⇒ split into 2, and DELIVERED');
});
...
test('MONSTER LINE: a single line longer than a frame is chopped up', () => {
  ...
  const oneLine = { id: 'mono', text: 'z'.repeat(9000), label: 'mono.md' };
  const p = planFrames([oneLine], 1200, 24);
  const emitted = p.flatMap((x) => x.emitted);
...
test('MONSTER LINE mixed with normal lines: the current buffer is flushed first', () => {
  ...
  const mixed = { id: 'mix', text: 'start\n' + 'z'.repeat(3000) + '\nfin', label: 'mix.md' };
  const p = planFrames([mixed], 1200, 24);
...
test('REMAINDER: EXACT message — ONE single announcement for BOTH paths', () => {
  ...
  const solo = plan(segments(6, 300), 1200);
...
test('FORCED PROGRESS: the frame SACRIFICES the announcement, but ALWAYS returns the remainder', () => {
  ...
  const onlyOne = p[0];
  assert.ok(onlyOne.emitted.length > 0, 'content GOES OUT anyway (progress guarantee)');
  assert.ok(onlyOne.deferred.length > 0, 'and the remainder is RETURNED to the caller');
  // The text, itself, carries NO announcement: there was no room.
  assert.ok(!onlyOne.text.includes('DEFERRED'), 'announcement sacrificed: delivering comes before describing');
  assert.ok(!onlyOne.text.includes('Stryker'), 'no stray label in the frame');
});
...
test('EMPTY FRAME: neither content nor remainder ⇒ STRICTLY empty rendering (silence)', () => {
  ...
  const p = planFrames(segments(6, 300), 1200, 4);
...
test('BOUNDED ANNOUNCEMENT: it counts DOCUMENTS and cannot eat the frame', () => {
  ...
  const five = planFrames(nDocs(6), 1500, 1);
  assert.ok(!five[0].text.includes('other(s)'), '5 deferrals ⇒ all cited, no truncation');
...
test('ORDER: the queue goes AHEAD of the fresh content (RFC 6455, never interleaved)', () => {
  ...
  const fresh = [{ id: 'b', text: 'B' }];
  assert.deepStrictEqual(orderSegments(file, fresh).map((s) => s.id), ['a#1', 'b']);
});
...
test('ORDER: a doc ALREADY in the queue is not re-stacked (dedup by DOCUMENT)', () => {
  ...
  const fresh = [{ id: 'a', text: 'A ENTIER' }, { id: 'b', text: 'B' }];
  assert.deepStrictEqual(orderSegments(file, fresh).map((s) => s.id), ['a#2', 'a#3', 'b']);
});
...
test('ORDER: the dedup compares DOCUMENTS on BOTH sides, not raw ids', () => {
  ...
  const fresh = [{ id: 'a#1', text: 'A1' }];
  assert.deepStrictEqual(orderSegments(file, fresh).map((s) => s.id), ['a']);
});
...
test('INFINITE BUDGET vs FLOOR: the same corpus defers at 8000 and NOT at infinity', () => {
  ...
  const segments = () => [{ id: 'g', text: 'Y'.repeat(76000), label: 'g' }];
  assert.ok(planFrames(segments(), 8000, 1)[0].deferred.length > 0, 'witness: at 8000 it overflows');
  assert.strictEqual(planFrames(segments(), Infinity, 1)[0].deferred.length, 0);
});
