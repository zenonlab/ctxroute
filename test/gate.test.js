...
test('cascade: decide() consumes owners — defaults.{source} has a real EFFECT', () => {
  ...
  const decls = { 'mcp/x': {} };
  const state = () => ({ 'mcp/x': { seen: true, sinceLastCall: 0 } });
  const withDefaults = decide(config, decls, ['mcp/x'], state(), 0, { 'mcp/x': 'mcp' });
  assert.deepStrictEqual(withDefaults.inject, ['mcp/x'], 'defaults.mcp = dumb => re-injects');
  const sans = decide(config, decls, ['mcp/x'], state(), 0, undefined);
...
test('enforce + dumb: block / pass / block… in REGULAR alternation', () => {
  ...
  const vus = [];
  for (let i = 0; i < 5; i++) {
    const r = decide({}, decls, ['d/x'], state, 0, own);
...
test('enforce + smart: blocks → passes again → RE-BLOCKS after N (the cadence, nothing else)', () => {
  ...
  let state = t2.state;
  for (let i = 0; i < 4; i++) state = decide({}, decls, ['other'], state, 0, { other: 'file' }).state;

  const t3 = decide({}, decls, ['d/x'], state, 0, own);
...
test('enforce inherits from defaults.{source} — and `false` CANCELS that inheritance', () => {
  ...
  const inherited = decide(config, { 'mcp/x': {} }, ['mcp/x'], {}, 0, { 'mcp/x': 'mcp' });
  assert.strictEqual(inherited.decision, 'deny', 'the category imposes the block');
  const unsubscribed = decide(config, { 'mcp/x': { enforce: false } }, ['mcp/x'], {}, 0, { 'mcp/x': 'mcp' });
  assert.strictEqual(unsubscribed.decision, 'allow', 'without an explicit `false`, an entry would be UN-OPT-OUT-ABLE');
});
...
test('the alternation SURVIVES a foreign action (the `denied` flag is propagated, not rebuilt)', () => {
  // 🔴 REAL DEFECT, fixed 2026-08-23. The foreign-action branch used to rebuild the
  //    entry as an object LITERAL `{ seen, sinceLastCall: n + 1 }` — dropping `denied`.
  //    So ANY tool call that did not match the doc erased the memory of the refusal, and
  //    the gesture the agent redoes was refused a SECOND time with no successful pass in
  //    between: exactly the infinite block "a block is NEVER followed by a block" forbids.
  //    The three conditions were met in PRODUCTION (`enforce: true` + `mode: smart` +
  //    driftUnit `tool`, the framework default).
  // ⚠️ The foreign action is what makes this cell bite: without it the flag is read from
  //    the previous entry and the alternation looks healthy. Never simplify it away.
  const decls = { 'd/x': { mode: 'smart', threshold: 1, enforce: true } };
  const own = { 'd/x': 'file' };
  const blocked = decide({}, decls, ['d/x'], {}, 0, own);
  assert.strictEqual(blocked.decision, 'deny', 'the 1st gesture is refused');
  assert.strictEqual(blocked.state['d/x'].denied, true, 'and the refusal is MEMORISED');
  const foreign = decide({}, decls, [], blocked.state, 0, own);
  assert.strictEqual(foreign.state['d/x'].denied, true, 'a foreign action must not erase it');
  assert.strictEqual(foreign.state['d/x'].sinceLastCall, 1, 'it only advances the drift');
  const resumed = decide({}, decls, ['d/x'], foreign.state, 0, own);
  assert.deepStrictEqual(resumed.inject, ['d/x'], 'the doc is still delivered');
  assert.strictEqual(resumed.decision, 'allow', 'the redone gesture PASSES — alternation held');
});

test('a foreign action propagates `turn` too (one quantity moves: the drift counter)', () => {
  // ⚠️ Same rebuild, same loss. `turn` costs no decision while a doc keeps its
  //    driftUnit, which is exactly why nothing saw it go — a silent field is the class
  //    this cell exists to keep closed, not this one field.
  const decls = { 'd/x': { mode: 'smart', threshold: 4 } };
  const own = { 'd/x': 'file' };
  const t1 = decide({}, decls, ['d/x'], {}, 7, own);
  assert.strictEqual(t1.state['d/x'].turn, 7);
  const t2 = decide({}, decls, [], t1.state, 9, own);
  assert.strictEqual(t2.state['d/x'].turn, 7, 'the last DELIVERY timestamp, untouched');
});
...
test('enforce works on ALL the sources of the registry (derived from ADAPTERS)', async () => {
  ...
  for (const src of ids) {
    ...
    const byDefault = decide({ defaults: { [src]: { enforce: true, mode: 'once' } } },
      { d: {} }, ['d'], {}, 0, { d: src });
    assert.strictEqual(byDefault.decision, 'deny', `source ${src}: defaults.${src}.enforce inert`);
    // ③ explicit opt-out
    const unsubscribed = decide({ defaults: { [src]: { enforce: true, mode: 'once' } } },
      { d: { enforce: false } }, ['d'], {}, 0, { d: src });
    assert.strictEqual(unsubscribed.decision, 'allow', `source ${src}: \`false\` does not opt out`);
  }
});
...
test('filter 52: PARITY — without filterMode, and without toolName, nothing changes (filteredOut empty)', () => {
  ...
  const filterWithoutTarget = decide({ filterMode: 'blacklist', filterList: ['Bash'] }, { d: DUMB }, ['d'], {}, 0, { d: 'file' });
  assert.deepStrictEqual(filterWithoutTarget.inject, ['d'], 'toolName absent = no target, the filter cannot bite');
});
