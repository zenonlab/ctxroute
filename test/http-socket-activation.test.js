...
test('the protocol is read exactly as sd_listen_fds(3) documents it', () => {
  ...
  for (const mauvais of ['3.5', ' 1 ', '0x1', '', 'yes', '-1']) {
    assert.strictEqual(inheritedFd({ LISTEN_PID: String(moi), LISTEN_FDS: mauvais }, moi), null,
      `LISTEN_FDS=${JSON.stringify(mauvais)} must not be read as a count`);
    assert.strictEqual(inheritedFd({ LISTEN_PID: mauvais, LISTEN_FDS: '1' }, moi), null,
      `LISTEN_PID=${JSON.stringify(mauvais)} must not be read as a pid`);
  }
...
test('SEEN RED: the criterion rejects the two ways this is got wrong', () => {
  ...
  const naïf = (/** @type {Record<string, string|undefined>} */ env) =>
    (Number(env.LISTEN_FDS) > 0 ? SD_LISTEN_FDS_START : null);
  assert.throws(() => critère(naïf), /ANOTHER process/,
  ...
  const muet = () => null;
  assert.throws(() => critère(muet), /not read/,
 ...
 */
function avecSocketHérité(opts) {
  return new Promise((resolve, reject) => {
    ...
    probe.listen(0, HOST, () => {
      ...
      child.stdout.on('data', (c) => {
        ...
        try {
          resolve({ child, annonce: JSON.parse(out.trim().split('\n')[0]), portSocket: port });
        } catch {
...
test.skipIf(!POSIX)('REAL: the daemon serves on the descriptor the OS passed it, never on the port', async () => {
  ...
  const { child, annonce, portSocket } = await avecSocketHérité({ selfPid: true, port: libre });
  try {
    assert.strictEqual(annonce.fd, SD_LISTEN_FDS_START, 'the child did not take the inherited descriptor');
    assert.strictEqual(annonce.port, portSocket,
    ...
    const réponse = await new Promise((resolve, reject) => {
    ...
    assert.deepStrictEqual(JSON.parse(réponse), {},
...
test.skipIf(!POSIX)('REAL: a LISTEN_PID belonging to another process is ignored, descriptor and all', async () => {
  ...
  const libre = await portLibre();
  const { child, annonce, portSocket } = await avecSocketHérité({ selfPid: false, port: libre });
  try {
    assert.strictEqual(annonce.fd, null, 'a descriptor owned by another pid was accepted');
    assert.strictEqual(annonce.port, libre, 'the fallback did not bind the configured port');
    assert.notStrictEqual(annonce.port, portSocket, 'the child served the inherited socket anyway');
  } finally {
    child.removeAllListeners('exit');
    child.kill();
  }
