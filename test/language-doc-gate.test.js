...
describe('gate: the language doc cannot contradict the code', () => {
  ...
  it('⑥ the PROSE restates no generated fact (else the copy re-drifts)', () => {
    ...
    // inside its block.
    const text = read();
    const inBlock = extract(text, 'cadence').content;
    const horsBloc = text.replace(inBlock, '');
    const modeList = MODES.map((m) => `\`${m}\``).join(' · ');
    expect(horsBloc.includes(modeList), 'the mode list is COPIED outside the AUTO block').toBe(false);
  });
});
