...
ok('serverName: a legitimate name with dashes is kept', () => lib.serverName('mcp__qa-seo__do') === 'qa-seo');
...
test('parseFrameArgs: an absurd input → a single frame, NEVER a throw', () => {
  ...
  for (const mauvais of [undefined, null, 'text', 42, {}]) {
    assert.deepStrictEqual(lib.parseFrameArgs(mauvais), { frame: 1, nbFrames: 1 });
  }
