 ...
 */
function faussServeur(echecs) {
  const journal = { listens: [], erreurs: [] };
  let restants = echecs.slice();
  return {
    journal,
    once(evt, cb) { if (evt === 'error') journal.erreurs.push(cb); },
...
test('a free address is taken directly — no probe, no unlink', async () => {
  ...
  bind(srv, '/tmp/x.sock', () => { ecoute = true; }, () => {}, {
    platform: 'darwin',
    probe: () => { sonde += 1; },
    unlink: () => { efface += 1; },
  });
...
test('a LIVING address is never cleared — the error reaches the caller as is', async () => {
  ...
  bind(srv, '/tmp/x.sock', () => {}, (err) => { recue = err; }, {
    platform: 'darwin',
  ...
  assert.equal(recue, EADDRINUSE, 'the refusal must reach the caller unchanged, never be swallowed');
