...
test('⚙️ TABLE OF ATOMS: each cell tells the truth, each blind cell is JUSTIFIED', () => {
  ...
  for (const c of CASES) {
    const measurement = c.withIt() !== c.withoutIt();
    if (measurement !== c.visible) faults.push(`${c.id}: declared ${c.visible ? 'VISIBLE' : 'BLIND'}, measured ${measurement ? 'VISIBLE' : 'BLIND'}`);
...
test('⚙️ ANTI-DORMANCY: the table really discriminates', () => {
  ...
  const blind = CASES.length - visible;
  assert.ok(visible >= 20, `suspicious visible cells: ${visible}`);
...
test('⚙️ NEGATIVE-CHECK: an amputated semantics would turn the table red', () => {
  ...
  const withoutFilters = (entry, payload) => {
  ...
  const withIt = !withoutFilters({ servers: ['srv'], exclude: ['aaa'] }, { toolName: 'mcp__srv__x', toolInput: { p: 'aaa' } });
  const withoutIt = !withoutFilters({ servers: ['srv'], exclude: ['aaa'] }, { toolName: 'mcp__srv__x', toolInput: { p: 'zzz' } });
