...
function triggered(text, payload, via) {
...
test('GATE: each trigger really TRIGGERS (real source call)', () => {
  ...
  for (const k of TRIGGERS) {
    ...
    assert.ok(triggered(c.fm, c.payload, c.via),
...
test('NEGATIVE-CHECK: the gate DETECTS a non-consumed trigger', () => {
  ...
  const sabotagedList = [...TRIGGERS, ghost];
  const missingOnes = sabotagedList.filter((k) => !cas[k]);
