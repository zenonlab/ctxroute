...
test('③ NO SECOND DEFINITION — nobody but the single source carries a harness root', () => {
  ...
  try {
    const controlFiles = controls(dir);
    const found = rebuilds(scan(perimeter().concat(controlFiles.map((c) => c.file)), repo), rs);
...
test('⑮ EVERY `*Label()` PUBLISHES A RELATIVE, POSIX, HOME-FREE ADDRESS THAT NO OVERRIDE CAN REWRITE', () => {
  ...
  const saved = { ...process.env };
  try {
    const before = labels.map((k) => mod[k]());
    ...
    assert.deepStrictEqual(apres, before,
