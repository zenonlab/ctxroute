...
const emit = (em, fresh) => em.emit({
  fresh, budgetMax: 8000, nbFrames: 1, index: 1, scopeId: SID,
});
...
test('SCOPE: two agents count SEPARATELY (never a global counter)', () => {
  ...
  const em = layer();
  emit(em, [seg('a')]);
  em.emit({ fresh: [seg('b')], budgetMax: 8000, nbFrames: 1, index: 1, scopeId: SID + '--agent-x' });
  assert.equal(em.emissionCount(SID), 1);
...
test('THE QUEUE SURVIVES THE COUNTER: both live in the SAME store without stepping on each other', () => {
  ...
  em.emit({ fresh: big, budgetMax: 900, nbFrames: 1, index: 1, scopeId: SID });
