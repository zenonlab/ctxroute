...
test('validate: a `rules` entry without a pattern, with an unknown key, or with a non-list scope = RED', () => {
  ...
  assert.ok(validate({ rules: [{ pattern: 'a.js', patern: 'b' }] }).length > 0, 'unknown key (typo patern)');
...
test('validate: `rules:` as a YAML block = RED + the message gives the canonical format', () => {
  ...
  const canonique = [{ pattern: 'foo.js' }, { pattern: 'bar.js', scope: ['project'] }];
...
test('SYMMETRY GATE ⓪: every behaviour key has a sample (nothing can hide)', () => {
  ...
  assert.ok(comportement.length >= 5, 'suspicious behaviour vocabulary');
});
...
const CORPUS_DECLARATIFS = ['file doc', 'MCP doc', 'skill entry'];
...
test('SYMMETRY GATE ②: an OPERATOR absent from a declarative corpus MUST be justified', async () => {
  ...
  for (const [k, v] of Object.entries(ECHANTILLON_MATCHING)) {
    ...
    const absents = CORPUS_DECLARATIFS.filter((c) => !presence[c]);
    const justif = ASYMETRIES_MATCHING[k];
...
test('SYMMETRY GATE ② bis: every operator of the vocabulary is PROBED (nothing can hide)', () => {
  ...
  for (const k of MATCHING) {
    assert.ok(k in ECHANTILLON_MATCHING || k in HORS_PERIMETRE,
      `\`${k}\` is an operator without a sample NOR a reason for exclusion: the gate does not see it.`);
  }
  assert.ok(Object.keys(ECHANTILLON_MATCHING).length >= 5, 'suspicious operator sample');
});
