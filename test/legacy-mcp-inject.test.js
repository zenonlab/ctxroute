...
// advances the counter of the current server (not only the native tools).
{
  ...
  ok('2 calls to ANOTHER server (B) ≥ threshold 2 → A re-injected', wasInjected(reinjectedA));
}
...
// NEVER cuts off the injection itself (additionalContext) ──
{
  ...
  ok('without a "showNotification" field → notification ON by default', hasNotification(withNotif));
  ok('without a "showNotification" field → injection present (normal behavior)', wasInjected(withNotif));
  ...
  ok('showNotification:false → NO systemMessage', !hasNotification(noNotif));
  ok('showNotification:false → injection STILL present (the toggle only cuts off the message)', wasInjected(noNotif));
