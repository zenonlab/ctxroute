...
test('DECLFOR ③: the SAMPLE covers every key (anti-blind-spot)', () => {
  ...
  assert.deepStrictEqual(nonCouvertes, [],
    'decision key(s) without a sample: ' + nonCouvertes.join(', ')
    + '\nAdd a VALID and NON-DEFAULT value to ECHANTILLON.');
  assert.ok(clesDeDecision().length >= 4, 'DORMANT gate: fewer than 4 derived keys');
});
...
test('DECLFOR ⑤ NEGATIVE: the part really sees, and it would really turn red', () => {
  ...
  const trouvees = sourcesAvecDeclFor();
  assert.ok(trouvees.length >= 2,
    'part ⑤ no longer inspects anything: ' + trouvees.length + ' source(s) found');

  // ② a source that RESOLVES is indeed detected as at fault
  const resolveuse = { declFor: () => ({ mode: 'smart', threshold: 4 }) };
  assert.ok(Object.keys(resolveuse.declFor({})).length > 0,
