...
const NARROWING = RULE_KEYS.filter((k) => !(k in NON_NARROWING));

const ATOM = 'demo-projet';
const NEVER = 'atome-absent-de-tout-geste';
...
const GESTE_SHELL = { toolName: 'Bash', toolInput: { command: `echo ${ATOM} >> memo.md` } };
const GESTE_MCP = { toolName: 'mcp__ssh__ssh_exec', toolInput: { command: `echo ${ATOM}` } };
...
const FRAGMENTS = () => ({
  scope: { base: {}, narrowed: { scope: [NEVER] } },
  exclude: { base: {}, narrowed: { exclude: [ATOM] } },
  keys: { base: { scope: [ATOM] }, narrowed: { scope: [ATOM], keys: ['-command'] } },
});
...
const parDoc = (header, geste) => {
  const text = `---\n${header}\nmode: dumb\n---\nBody.\n`;
  return fileSrc.matchingDocs(rulesFromCorpus([{ doc: 'd.md', text: text }]), geste).length > 0;
};
...
const CELLS = () => [
  {
    ...
    decide: (f) => parDoc(`match: ${ATOM}\n${yaml(f)}`, GESTE_SHELL),
  },
  {
    // Same road, per-entry `rules` form (its operators live INSIDE the entry).
    id: 'corpus/frontmatter-rules',
    decide: (f) => parDoc(`rules: ${JSON.stringify([{ pattern: ATOM, ...f }])}`, GESTE_SHELL),
  },
  {
    id: 'file/pattern',
    decide: (f) => fileSrc.matchingDocs([{ pattern: ATOM, doc: 'd.md', ...f }], GESTE_SHELL).length > 0,
  },
  {
    id: 'tool/frontmatter',
  ...
  },
  {
    ...
    id: 'skill/match',
    decide: (f) => skill({ match: [ATOM], ...f }, GESTE_SHELL),
  },
  {
    // Narrowing goes INSIDE the rule here: a per-entry form has its own operators.
    id: 'skill/rules',
    decide: (f) => skill({ rules: [{ pattern: ATOM, ...f }] }, GESTE_SHELL),
  },
  {
...
test('⓪ every per-rule key is CLASSIFIED (narrowing, or excluded with a reason)', () => {
  for (const k of RULE_KEYS) {
    assert.ok(
      NARROWING.includes(k) || k in NON_NARROWING,
      `\`${k}\` is neither declared narrowing nor excluded with a reason — classify it`,
    );
  }
  assert.ok(NARROWING.length >= 3, `suspicious derivation: ${NARROWING.length} narrowing operators`);
});
...
test('① every narrowing operator is CONSUMED on EVERY dimension (probed, not read)', () => {
  ...
  for (const cell of CELLS()) {
    for (const op of NARROWING) {
      ...
      if (cell.decide(frag.narrowed) !== false) dead.push(`${cell.id} ignores \`${op}\``);
    }
  }
  assert.deepEqual(
    dead, [],
...
test('② the MCP corpus REFUSES every narrowing operator (never accepted-and-inert)', () => {
  for (const op of NARROWING) {
    const errors = validateMcp({ [op]: ['x'] });
    assert.ok(
      errors.length > 0 && errors[0].includes(op),
...
test('③ NEGATIVE-CHECK: a dimension that DROPS an operator is detected', () => {
  ...
  const sabotaged = {
    ...
    decide: (f) => {
      ...
      return skill({ match: [ATOM], ...sansKeys }, GESTE_SHELL);
    },
  };
  assert.equal(sabotaged.decide(frags.keys.base), true, 'the sabotaged base must still inject');
  assert.equal(
    sabotaged.decide(frags.keys.narrowed), true,
