...
const FLEET_SKILL = path.join(os.homedir(), '.claude', 'commands', 'ctxroute.md');
const FLEET_DOCS = path.join(os.homedir(), '.claude', 'hooks', 'docs', 'ctxroute');

const fleetExists = fs.existsSync(FLEET_DOCS) && fs.existsSync(FLEET_SKILL);
...
test.skipIf(!fleetExists)('the repo SKILL.md == the wired fleet skill (drift = RED, copy it over)', () => {
  assert.strictEqual(read(path.join(REPO_DIR, 'SKILL.md')), read(FLEET_SKILL),
...
test.skipIf(!fleetExists)('every injectable fleet doc has its IDENTICAL mirror in the repo (none forgotten)', () => {
  for (const f of fs.readdirSync(FLEET_DOCS).filter((x) => x.endsWith('.md'))) {
    ...
    assert.strictEqual(read(mirror), read(path.join(FLEET_DOCS, f)),
      `fleet↔repo drift on ${f} — copy it over in the same gesture as the edit.`);
  }
});

test.skipIf(!fleetExists)('no orphan repo file (doc deleted from the fleet but left in the repo)', () => {
  const fleetFiles = new Set(fs.readdirSync(FLEET_DOCS).filter((x) => x.endsWith('.md')));
  for (const f of fs.readdirSync(REPO_DIR).filter((x) => x.endsWith('.md') && x !== 'SKILL.md')) {
    assert.ok(fleetFiles.has(f), `orphan repo file: docs/framework/${f} (absent from the fleet) — delete it or re-wire it.`);
  }
});
...
const FLEET_HOOKS = path.join(os.homedir(), '.claude', 'hooks');
...
const ABSOLUTE_PATH = /['"]([A-Za-z]:[\\/][^'"]+)['"]/g;

function canaryRenderers() {
  if (!fs.existsSync(FLEET_HOOKS)) return [];
  return fs
    .readdirSync(FLEET_HOOKS)
    .filter((f) => f.endsWith('.js'))
    .map((f) => ({ name: f, src: read(path.join(FLEET_HOOKS, f)) }))
...
function brokenContract(renderers, canarySrc, checkSrc) {
  const brokenOnes = [];
  for (const a of renderers) {
    // ① PATHS — the REAL failure of 04→07/08.
    for (const m of a.src.matchAll(ABSOLUTE_PATH)) {
    ...
    for (const m of a.src.matchAll(/const\s*\{([^}]*)\}\s*=\s*require\([^)]*canary\.js[^)]*\)/g)) {
      for (const name of m[1].split(',').map((s) => s.trim()).filter(Boolean)) {
        if (!exportsDe(canarySrc).has(name)) {
  ...
  if (renderers.some((a) => /\.verdict\b/.test(a.src)) && !/verdict:/.test(checkSrc)) {
    brokenOnes.push('canary-check.js no longer writes the `verdict` key the display reads');
  }
  return brokenOnes;
}

const CANARY_SRC = read(path.join(import.meta.dirname, '..', 'src', 'canary.js'));
const CHECK_SRC = read(path.join(import.meta.dirname, '..', 'src', 'hooks', 'canary-check.js'));

test.skipIf(!fleetExists)('④ canary ⟷ display contract: paths, export and verdict key hold', () => {
  const renderers = canaryRenderers();
  if (renderers.length === 0) return; // no display = a legitimate choice
  const brokenOnes = brokenContract(renderers, CANARY_SRC, CHECK_SRC);
...
test('④ NEGATIVE: the part really goes red on the 3 breaches (IN-MEMORY sabotage)', () => {
  ...
  const staleEntry = [{ name: 'fakeOnes.js', src: "require('C:/Users/dev/Desktop/mcp-doc-hooks/canary.js')" }];
  assert.strictEqual(brokenContract(staleEntry, CANARY_SRC, CHECK_SRC).length, 1,
  ...
  assert.deepStrictEqual(brokenContract(alive, CANARY_SRC, CHECK_SRC), [],
    'false positive on a path that does exist');

  // ② An export that does not exist.
  const ghostExport = [{ name: 'fakeOnes.js', src: "const { marqueurFantome } = require('../src/canary.js');" }];
  assert.match(brokenContract(ghostExport, CANARY_SRC, CHECK_SRC)[0] || '', /does not export/,
    'the part does not see a vanished export');

  // ③ The verdict key renamed on the shell side.
  const reader = [{ name: 'fakeOnes.js', src: 'const v = JSON.parse(x); v.verdict;' }];
  assert.strictEqual(brokenContract(reader, CANARY_SRC, 'fs.writeFileSync(t, JSON.stringify({ etat: v }))').length, 1,
  ...
  assert.deepStrictEqual(brokenContract(healthy, CANARY_SRC, CHECK_SRC), [],
