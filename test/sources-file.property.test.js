...
test('LAW ① MONOTONICITY — adding a pattern to `exclude` can only REDUCE what is injected', () => {
  fc.assert(
    fc.property(word(), fc.array(word(), { maxLength: 3 }), word(), geste(), (pattern, ex, deplus, payload) => {
      const avant = docsDe([{ pattern, doc: 'd.md', exclude: ex }], payload);
      const after = docsDe([{ pattern, doc: 'd.md', exclude: [...ex, deplus] }], payload);
      assert.ok(inclus(after, avant), `a BROADENED exclude ADDED an injection: ${JSON.stringify({ pattern, ex, deplus, payload })}`);
    }),
    { numRuns: 3000 }
  );
});
...
test('LAW ② INDEPENDENCE FROM WRITING — enriching the gesture never RE-AUTHORIZES an excluded one', () => {
  ...
  fc.assert(
    fc.property(word(), fc.array(word(), { minLength: 1, maxLength: 2 }), commande(), word(), fc.nat(), (pattern, ex, cmd, insere, pos) => {
      ...
      words.splice(pos % (words.length + 1), 0, insere);
      const enrichi = words.join(' ');
      const after = docsDe(rules, { toolName: 'Bash', toolInput: { command: enrichi } });
      assert.deepStrictEqual(after, [], `the word "${insere}" RE-AUTHORIZED: ${enrichi}`);
    }),
    { numRuns: 5000 }
  );
});
...
test('LAW ③ (㊺①) — adding a GROUP to `scope` can only REDUCE what is injected', () => {
  ...
  fc.assert(
    fc.property(word(), fc.array(fc.array(word(), { minLength: 1, maxLength: 2 }), { minLength: 1, maxLength: 3 }), fc.array(word(), { minLength: 1, maxLength: 2 }), geste(), (pattern, groupes, deplus, payload) => {
      const avant = docsDe([{ pattern, doc: 'd.md', scope: groupes }], payload);
      const after = docsDe([{ pattern, doc: 'd.md', scope: [...groupes, deplus] }], payload);
      assert.ok(inclus(after, avant), `one more GROUP BROADENED: ${JSON.stringify({ pattern, groupes, deplus })}`);
    }),
    { numRuns: 3000 }
  );
});
