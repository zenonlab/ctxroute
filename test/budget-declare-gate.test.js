...
const WIRING = 'C:/ProgramData/OpenAI/Codex/requirements.toml';
const present = fs.existsSync(WIRING);
...
const emitters = (bs) => bs.filter((b) => b.limit !== null);

test.skipIf(!present)('GATE: each Codex emitter DECLARES its budget to the engine', () => {
  const sans = emitters(blocs(fs.readFileSync(WIRING, 'utf8')))
...
test.skipIf(!present)('GATE: the declared limit and the passed budget are the SAME number', () => {
  const discrepancies = emitters(blocs(fs.readFileSync(WIRING, 'utf8')))
    .filter((b) => b.budget !== null && b.budget !== b.limit)
    .map((b) => `${b.script}: additionalContextLimit=${b.limit} but --budget ${b.budget}`);
  assert.deepStrictEqual(
    discrepancies,
    [],
...
test('NEGATIVE: a divergence is DETECTED (gate not inert)', () => {
  ...
  const mute = healthy.replace(' --budget 0', '');

  assert.strictEqual(emitters(blocs(healthy)).filter((b) => b.budget !== b.limit).length, 0,
    'witness: a coherent wiring does not go red');
  assert.strictEqual(emitters(blocs(divergent)).filter((b) => b.budget !== b.limit).length, 1,
    'SABOTAGE NOT DETECTED: two different numbers would go green.');
  assert.strictEqual(emitters(blocs(mute)).filter((b) => b.budget === null).length, 1,
