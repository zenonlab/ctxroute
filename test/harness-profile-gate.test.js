...
const KERNEL = ['src/sources/file.js', 'src/sources/tool.js', 'src/sources/mcp.js', 'src/sources/skill.js', 'src/sources/session.js', 'src/gate.js', 'src/budget.js', 'src/loader.js', 'src/collisions.js', 'src/lint.js'];

// The DIALECT VOCABULARY = every value of the profile, flattened.
const dialect = Object.values(DEFAULT_PROFILE).flat();
...
test('㊽ ANTI-DORMANCY — the profile really declares a vocabulary, and the scan really reads', () => {
  ...
  assert.ok(dialect.length >= 4, `suspicious dialect vocabulary: ${dialect.length}`);
  assert.ok(dialect.every((d) => typeof d === 'string' && d.length > 0));
  for (const f of KERNEL) assert.ok(read(f).length > 200, `empty or unreadable core file: ${f}`);
});
...
test('㊽ NEGATIVE-CHECK — the gate BITES on a reintroduction (IN-MEMORY sabotage)', () => {
  ...
  const sabotaged = codeOnly(`const x = '${dialect[0]}';`);
  assert.ok(dialect.some((m) => sabotaged.includes(`'${m}'`)), 'the gate would NOT detect a reintroduction');
  // And it does NOT bite on a mention in a COMMENT (otherwise every code doc would turn red).
  const comment = codeOnly(`// we no longer test '${dialect[0]}' here\nconst y = 1;`);
  assert.ok(!dialect.some((m) => comment.includes(`'${m}'`)), 'a comment must NEVER turn red');
});
