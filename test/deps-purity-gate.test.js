...
test('REAL SABOTAGE: each "pure" module makes its gate GO RED when impurified', () => {
  ...
  assert.ok(cibles.length >= 2, 'premise: several modules are declared pure');
  ...
  for (const cible of cibles) {
    ...
    fs.mkdirSync(path.dirname(path.join(bac, cible)), { recursive: true });
    fs.writeFileSync(
      path.join(bac, cible),
    ...
    assert.match(
      output, new RegExp(cible.replace('.', '\\.') + '.*fs|must-stay-pure'),
      `${cible} was impurified and NO gate went red — the rule protecting it is INERT.`,
    );
  }
}, 120000);
