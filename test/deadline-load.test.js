...
const LEGACY = path.join(FLEET, 'protect-files.js');
...
function matchingPath() {
  const { rules } = JSON.parse(fs.readFileSync(path.join(FLEET, 'protected-paths.json'), 'utf8'));
...
function launched(payload) {
  return new Promise((resolve) => {
    const c = execFile(process.execPath, [LEGACY], { cwd: FLEET, encoding: 'utf8' }, (_e, out) => resolve(out || ''));
    c.stdin.end(payload);
  });
}

test.skipIf(skip)('UNDER LOAD — the armed hooks ALWAYS inject (0 empty output)', { timeout: 60000 }, async () => {
  const target = matchingPath();
  ...
  const outputs = await Promise.all(Array.from({ length: N }, () => launched(payload)));

  const emptyOnes = outputs.filter((o) => !o.includes('[source:')).length;
  assert.strictEqual(
    emptyOnes,
    0,
