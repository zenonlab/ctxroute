...
function runIn(payload, { env, args = [] } = {}) {
  ...
  const avant = {};
  for (const [k, v] of Object.entries(e)) { avant[k] = process.env[k]; process.env[k] = v; }
  let out = null;
  try {
    const data = payload || {};
...
test('FRAMES: 3 docs too big for one frame → delivered in 3 frames, NOTHING evicted', async () => {
  ...
  sorties.forEach((s, i) => assert.ok(s.includes('FRAME ' + (i + 1) + '/3'), 'frame ' + (i + 1) + ' numbered'));
  const markers = sorties.map((s) => /###END:([0-9a-f]{8})###/.exec(s)[1]);
...
test('QUEUE: a corpus bigger than the capacity of ONE call arrives WHOLE over several actions', async () => {
  ...
  for (let k = 0; k < 6; k++) {
    ...
    assert.ok(tout.includes(lettre.repeat(700)), 'doc f' + k + ' eventually arrives IN FULL');
  }
