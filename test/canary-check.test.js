...
const harnessNoise = () => '{"type":"tool_use","name":"Read"}\n';
...
test("REAL SPAWN: one LANDED injection ⇒ verdict 'alive', whatever the volume emitted", () => {
  ...
  fs.writeFileSync(t, harnessNoise().repeat(40) + injection());
  poserEmissions(d, 40);
  launch({ transcript_path: t, session_id: SID }, d);
  const health = JSON.parse(fs.readFileSync(path.join(d, 'canary.json'), 'utf8'));
  assert.equal(health.verdict, 'alive');
  assert.equal(health.injections, 1);
});
...
test('REAL SPAWN — THE CASE THAT JUSTIFIES EVERYTHING: channel DEAD ⇒ verdict "dead"', () => {
  ...
  const d = tmp();
  const t = path.join(d, 'transcript.jsonl');
  fs.writeFileSync(t, harnessNoise().repeat(EMISSIONS_THRESHOLD));
  poserEmissions(d, EMISSIONS_THRESHOLD);
  launch({ transcript_path: t, session_id: SID }, d);
  const health = JSON.parse(fs.readFileSync(path.join(d, 'canary.json'), 'utf8'));
  assert.equal(health.verdict, 'dead');
  assert.equal(health.emissions, EMISSIONS_THRESHOLD);
  assert.equal(sourceTag(health.verdict), '💉⚠️ INJECTION DEAD');
});
...
test("REAL SPAWN — NEGATIVE: a NOISY transcript without emissions NEVER accuses", () => {
  ...
  const d = tmp();
  const t = path.join(d, 'transcript.jsonl');
  fs.writeFileSync(t, harnessNoise().repeat(EMISSIONS_THRESHOLD * 10));
  // No `poserEmissions`: the framework emitted nothing.
  launch({ transcript_path: t, session_id: SID }, d);
  const health = JSON.parse(fs.readFileSync(path.join(d, 'canary.json'), 'utf8'));
  assert.equal(health.verdict, 'undecidable');
  assert.equal(health.emissions, 0);
  assert.equal(sourceTag(health.verdict), '', 'silence is mandatory as long as nothing proves the failure');
});
...
test('REAL SPAWN: BOUNDED read — a big transcript does not cost its size', () => {
  ...
  const padding = harnessNoise().repeat(Math.ceil((BYTE_WINDOW * 1.5) / harnessNoise().length));
  fs.writeFileSync(t, injection().repeat(1000) + padding);
  assert.ok(fs.statSync(t).size > BYTE_WINDOW, 'premise: the file exceeds the window');
  poserEmissions(d, EMISSIONS_THRESHOLD);
  launch({ transcript_path: t, session_id: SID }, d);
  const health = JSON.parse(fs.readFileSync(path.join(d, 'canary.json'), 'utf8'));
  assert.equal(health.verdict, 'dead', 'old injections outside the window do not mask the ONGOING failure');
});
...
test("BOUNDARY CONTRACT: the canary reads the key THAT the emission layer writes", () => {
  ...
  const d = tmp();
  const t = path.join(d, 'transcript.jsonl');
  fs.writeFileSync(t, harnessNoise());
  ...
  const write = `
    ...
    for (let i = 0; i < ${EMISSIONS_THRESHOLD}; i++) {
      em.emit({ fresh: [{ id: 'd' + i, text: 'x' }], budgetMax: 8000, nbFrames: 1, index: 1, scopeId: ${JSON.stringify(SID)} });
    }
  `;
  ...
  const health = JSON.parse(fs.readFileSync(path.join(d, 'canary.json'), 'utf8'));
  assert.equal(
    health.emissions, EMISSIONS_THRESHOLD,
...
test("LOG: the verdict says WHICH transcript was read, and HOW MUCH", () => {
  ...
  assert.equal(health.transcript, t);
  // its real size, and how much of it was actually read
  assert.equal(health.tailleTranscript, fs.statSync(t).size);
  assert.equal(health.octetsLus, Math.min(BYTE_WINDOW, fs.statSync(t).size));
...
test('LOG: it describes the CURRENT run, never a frozen value', () => {
  ...
  fs.writeFileSync(big, harnessNoise().repeat(500) + injection());
