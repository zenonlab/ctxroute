...
function scanRepo(racine, m) {
  ...
  for (const rel of trackedFiles(racine)) {
    let text;
    try {
      text = fs.readFileSync(path.join(racine, rel), 'utf8');
    } catch {
      continue; // binary or unreadable: out of scope
    }
    for (const v of scan(text, m)) violations.push(`${rel} → ${v.name} (${v.excerpt})`);
  }
  return violations;
}
...
const CLIENTS_ROOT_NAME = /^clients(-.*)?$/i;

function seenRoots(zones) {
  const seen = [];
  for (const zone of zones) {
    ...
    for (const e of entrees) {
      ...
      const filePath = path.join(zone, e.name);
      // A DIRECT root (`.../client-sites`) or a project carrying its own
      // (`.../content-agent-local/clients`) — the EXACT case that leaked.
      if (CLIENTS_ROOT_NAME.test(e.name)) { seen.push(filePath); continue; }
      try {
        for (const sous of fs.readdirSync(filePath, { withFileTypes: true })) {
          if (sous.isDirectory() && CLIENTS_ROOT_NAME.test(sous.name)) {
            seen.push(path.join(filePath, sous.name));
          }
        }
...
test('② every client root is DECIDED — derived, or ignored with its reason', () => {
  ...
  const derived = (decl.derivedFolders || []).map((s) => s.root);
  const ignored = (decl.ignoredRoots || []).map((s) => s.root);
  const zones = [...new Set(derived.map((r) => path.dirname(r)))];
  const forgotten = forgottenRoots(seenRoots(zones), derived, ignored);
  assert.deepEqual(
    forgotten,
    [],
...
test('the gate only looks at what is TRACKED', () => {
  ...
  const tracked = trackedFiles(ICI);
  ...
  assert.ok(tracked.length > 200, `git ls-files answered ${tracked.length} files — the scan is not covering the repository`);
  assert.ok(!tracked.includes('ctxroute-config.json'), 'the user config stays gitignored');
});
...
test('NEGATIVE-CHECK: the gate KNOWS how to redden (sabotage on a COPY)', () => {
  ...
  try {
    ...
    git(['init', '-q'], bac);
    fs.writeFileSync(path.join(bac, 'piege.md'), 'author: ' + TERM + '\n');
    ...
    fs.writeFileSync(list, JSON.stringify({ terms: [TERM], derivedFolders: [] }));
    ...
    try {
      ...
      if (before === undefined) delete process.env.CTXROUTE_LEAK_LIST;
      else process.env.CTXROUTE_LEAK_LIST = before;
    }
  } finally {
...
test('scan: TOTAL — absurd inputs, never a throw', () => {
  ...
  for (const wrong of [undefined, null, 42, {}, []]) {
    ...
    assert.deepEqual(scan('texte', wrong), []);
  }
});
