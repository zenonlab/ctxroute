...
const FILE_DOCS = path.join(TMP, 'filedocs');

function run(payload) {
  return new Promise((resolve) => {
    const child = execFile(process.execPath, [HOOK], {
      encoding: 'utf8',
      env: { ...process.env, CTXROUTE_FILEDOCS_DIR: FILE_DOCS },
...
beforeEach(() => {
  fs.rmSync(FILE_DOCS, { recursive: true, force: true });
  fs.mkdirSync(FILE_DOCS, { recursive: true });
});

afterAll(() => fs.rmSync(TMP, { recursive: true, force: true }));

test('BLOCK: apply_patch (command) on an INVALID doc of the fleet → decision block + reason', async () => {
  const doc = path.join(FILE_DOCS, 'cassee.md');
...
test('SILENCE: HEALTHY doc of the fleet → empty stdout', async () => {
  const doc = path.join(FILE_DOCS, 'healthyOne.md');
  fs.writeFileSync(doc, '---\nmatch: server.js\nmode: dumb\n---\ncontent\n');
  const { code, stdout } = await run({ tool_name: 'apply_patch', tool_input: { command: patchFor(doc) } });
  assert.strictEqual(code, 0);
...
test('MULTI-FILE: patch touching a healthy doc THEN a broken one → block on the broken one', async () => {
  const healthyOne = path.join(FILE_DOCS, 'healthyOne.md');
  const broken = path.join(FILE_DOCS, 'cassee.md');
  fs.writeFileSync(healthyOne, '---\nmatch: a.js\nmode: dumb\n---\nok\n');
  fs.writeFileSync(broken, '---\nmach: typo.js\n---\nko\n');
  const { stdout } = await run({ tool_name: 'apply_patch', tool_input: { command: patchFor(healthyOne, broken) } });
  const out = JSON.parse(stdout);
  assert.strictEqual(out.decision, 'block');
  assert.ok(out.reason.includes('cassee.md'));
});
