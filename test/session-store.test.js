...
test('ATOMICITY: no temporary file survives the writes', { timeout: 30000 }, () => {
  ...
  assert.strictEqual(run().restes, 0, 'abandoned .tmp file(s) in state/');
});
...
test('ONLY absence is retried — a real error is NOT swallowed by repetition', () => {
  let appels = 0;
  const refusal = () => { appels += 1; const e = new Error('EPERM'); e.code = 'EPERM'; throw e; };
  assert.deepStrictEqual(store.readThrough(refusal, 'x'), {});
  assert.strictEqual(appels, 1, 'an EPERM must be decided ON THE SPOT, never retried into silence');

  let lectures = 0;
  const truncated = () => { lectures += 1; return '{ coupé'; };
  assert.deepStrictEqual(store.readThrough(truncated, 'x'), {});
