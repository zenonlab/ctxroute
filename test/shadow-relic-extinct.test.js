...
const FOLDERS = ['src', 'test', 'tools', 'docs/framework', '.github'];
const ROOT_FILES = [
  'FILE-MAP.md',
  'package.json',
  'vitest.config.mjs',
...
// ⚠️ THE STEERING JOURNALS ARE SCANNED THROUGH `paths.js`, NEVER FROM `ROOT`
//    (2026-08-22). `REFACTOR-PLAN.md` is gitignored, so it follows no branch,
//    no merge and no checkout: read at `<worktree>/REFACTOR-PLAN.md` it was a
//    DIFFERENT PHYSICAL FILE per worktree, and this gate went RED on a citation
//    already deleted in the other tree. One copy, in the repository's COMMON
//    directory, for every worktree. A resolution failure yields NO path — never
//    a fallback to a plausible per-tree file, which is the defect itself.
// ⚠️ THE ACTIVE PLAN ONLY — `REFACTOR-ARCHIVE.md` IS DELIBERATELY OUT, and the
//    reason was MEASURED: adding it turned this cell RED on a truthful line.
//    The archive is the record of what has been CLOSED, so it legitimately
//    names files that no longer exist — accusing it would be accusing history,
//    and a gate red on the truth gets unplugged the same day (the exact reason
//    `backlog-coherence-gate` anchors itself on the FIRST open table).
const STEERING_FILES = (() => {
  try { return [paths.planPath()]; } catch { return []; }
})();
...
test('NOTHING DESIGNATES IT ANY MORE — zero surviving citation in a tracked file', () => {
  const files = FOLDERS.reduce((acc, d) => lister(path.join(ROOT, d), acc), []);
  for (const f of ROOT_FILES) {
    const abs = path.join(ROOT, f);
    if (fs.existsSync(abs)) files.push(abs);
  }
  for (const abs of STEERING_FILES) {
    if (fs.existsSync(abs)) files.push(abs);
  }
  // ⚠️ FLOOR — a scan of an empty set proves nothing at all.
  assert.ok(files.length > 50, `hollow scan: only ${files.length} files read, the gate is measuring nothing`);

  const offenders = [];
  for (const abs of files) {
    if (abs.endsWith('shadow-relic-extinct.test.js')) continue; // this file names them ON PURPOSE
    if (CITATION.test(fs.readFileSync(abs, 'utf8'))) offenders.push(path.relative(ROOT, abs));
  }
