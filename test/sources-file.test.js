...
test('㊵ shouldSkip: scope sees a NESTED param (REAL shape of every MCP)', () => {
  ...
  assert.strictEqual(shouldSkip(r, 'mcp__mailer__mailer_call', payload), false);
});
...
test('㊵.a textValues: the corpus REAL depth (11) is well within the bound', () => {
  // ⚠️ MEASUREMENT 12/08/2026: 25,898 real calls, max depth = 11 (mailer).
  assert.deepStrictEqual(textValues(nest(11, 'found')).chunks, ['found']);
  assert.strictEqual(textValues(nest(11, 'found')).truncated, null);
});
test('㊵.a textValues: beyond MAX_DEPTH it truncates, AND it SAYS so', () => {
  const r = textValues(nest(MAX_DEPTH + 1, 'perdu'));
...
test('㊵.a textValues: the FIRST truncation reason is kept', () => {
  ...
  const r = textValues({ p: nest(MAX_DEPTH + 1, 'x'), t: 'y'.repeat(MAX_SIZE + 1) });
  assert.strictEqual(r.truncated, 'depth');
});
...
test('51 MUTANT — the trigger DEPTH BOUND really bites', () => {
  ...
  const dig = (n, leaf) => (n === 0 ? leaf : { a: dig(n - 1, leaf) });
  assert.deepStrictEqual(extractFilePaths('X', dig(3, { file_path: '/a/vu.js' })), ['/a/vu.js']);
  assert.deepStrictEqual(extractFilePaths('X', dig(MAX_DEPTH + 2, { file_path: '/a/trop-loin.js' })), [],
...
test('51 MUTANT — the BOUND is EXACT (the last admitted level, and the first refused)', () => {
  ...
  const dig = (n, leaf) => (n === 0 ? leaf : { a: dig(n - 1, leaf) });
  const found = (n) => extractFilePaths('X', dig(n, { path: '/x.js' })).length > 0;
