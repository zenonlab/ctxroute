...
test('reassemble: a divergence in the LAST chunk turns it RED (the prefix could not see it)', () => {
  ...
  const saboted = texts.slice();
  saboted[last] = texts[last].replace('line 120', 'LINE 120');
  // ANTI-VACUITY: the sabotage must land in the LAST frame, nowhere else.
  assert.notStrictEqual(saboted[last], texts[last], 'the sabotage did not touch the last frame');
  const r = reassemble(saboted);
  assert.ok(r.ok, r.reason);
...
test('reassemble: an envelope it cannot place is REFUSED, never compared anyway', () => {
  ...
  const bancal = s.replace(`###END:${mk}###`, '###END:00000000###');
  assert.notStrictEqual(bancal, s, 'the sabotage must really change the HEAD marker');
  assert.strictEqual(reassemble([bancal]).reason, 'frame 1 carries a seal this reader cannot parse');
