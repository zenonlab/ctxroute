...
//    and part ④ would fall.
const WITNESS_TOOL_NAME = 'mcp__' + T + '__appel';

const PROJECTIONS = {
  'tool name': () => ({ toolName: WITNESS_TOOL_NAME, toolInput: {} }),
...
const SOURCES = {
  ...
  tool: (payload) =>
    toolSource.matchingDocs([{ doc: 'd', fm: { tool: [WITNESS_TOOL_NAME] } }], payload).length > 0
  ...
  skill: (payload) =>
    ...
    || skillSource.matchingSkills({ skills: { s: { tool: [WITNESS_TOOL_NAME] } } }, payload).length > 0
...
test('SUBSTRATE ①: every BLINDNESS of a source on a projection MUST be justified', () => {
  ...
  for (const [sourceName, probe] of Object.entries(SOURCES)) {
    for (const [projName, payload] of Object.entries(PROJECTIONS)) {
      ...
      const justification = JUSTIFIED_BLIND_SPOTS[key];
      if (typeof justification !== 'string' || justification.trim().length <= 40) missingOnes.push(key);
    }
  }
  assert.deepEqual(missingOnes, [],
...
test('SUBSTRATE ②: a STALE justification must disappear (reverse part)', () => {
  ...
  //    engine.
  const staleOnes = [];
  for (const key of Object.keys(JUSTIFIED_BLIND_SPOTS)) {
...
test('SUBSTRATE ④: the WITNESS really discriminates (anti-lying-probe)', () => {
  ...
  const withoutWitness = () => ({ toolName: 'Read', toolInput: { file_path: '/rien/du/tout.js' }, cwd: '/ailleurs' });
  for (const [sourceName, probe] of Object.entries(SOURCES)) {
    assert.equal(probe(withoutWitness()), false,
