...
test('every entry carries a WITNESS, and the witness really witnesses', () => {
  ...
  for (const obs of DERIVED_OBSERVABLES) {
    assert.ok(typeof obs.witness === 'function' && typeof obs.witness().command === 'string' && typeof obs.witness().pattern === 'string',
      `${obs.name}: no witness — the reach table would build a cell that measures nothing`);
    const cmd = obs.witness().command.split('@@').join('atome');
    const motif = obs.witness().pattern.split('@@').join('atome');
...
test('the names are UNIQUE and DERIVED — two facts under one name is the defect itself', () => {
  ...
  const harnessKeys = [...DEFAULT_PROFILE.pathKeys, ...DEFAULT_PROFILE.commandKeys, ...DEFAULT_PROFILE.contentKeys];
  for (const n of DERIVED_NAMES) {
    assert.ok(!harnessKeys.includes(n), `${n} collides with a harness key — one of the two becomes unaddressable`);
  }
});
...
test('the DIRECTORY is a PREFIX of every reconstructed path — the containment that makes the split honest', () => {
  ...
  for (const filePath of commandPathCandidates(cmd)) {
    assert.ok(filePath.startsWith(commandCwdCandidates(cmd)[0]), 'a reconstructed path no longer carries its directory');
  }
});
...
test('LAW — every candidate is a NON-EMPTY string: an empty one would match EVERY pattern', () => {
  ...
  fc.assert(fc.property(command(), (c) => {
    for (const candidate of derivedCandidates(c)) {
...
test('CASE — the empty-candidate law, deterministic (properties are outside the Stryker runner)', () => {
  for (const cmd of ['cd /w/p && ', 'cd /w/p &&', 'cd /w/p ;  ; ']) {
    for (const candidate of derivedCandidates(cmd)) assert.ok(candidate.length > 0, `empty candidate on ${JSON.stringify(cmd)}`);
  }
});
