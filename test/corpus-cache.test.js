...
describe('corpus cache', () => {
  ...
  it('treats a non-positive ceiling as ABSENT and falls back to the declared default', () => {
    const commonDir = corpusRoot('v1');
    const stateDirectory = skillsRoot('BODY-V1');
    process.env.CTXROUTE_SKILLS_DIR = stateDirectory;

    const w1 = recordingWatch();
    enableCache(w1.watch, 0);
    expect(readCorpus(commonDir, 'docs/')[0].text).toBe('v1');
