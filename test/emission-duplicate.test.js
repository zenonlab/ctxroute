...
const emittedIds = (frames) => frames.flatMap((p) => p.emitted || []);
...
test('① an EMITTED segment never stays in the queue (the defect of 06/08/2026)', () => {
  isolated(() => {
    const em = emission.emit({
      fresh: grosDoc(), budgetMax: 8000, nbFrames: 12, index: 1, scopeId: 'S',
    });
    const enFile = idsEnFile(em.frames);
    const duplicates = emittedIds(em.frames).filter((id) => enFile.includes(id));
    assert.deepStrictEqual(duplicates, [], 'emitted AND in the queue: ' + duplicates.join(', '));
  });
});
...
test('② no segment is emitted twice within the same action', () => {
  isolated(() => {
    ...
    const vus = new Set();
    const repeated = emittedIds(em.frames).filter((id) => (vus.has(id) ? true : (vus.add(id), false)));
    assert.deepStrictEqual(repeated, [], 'emitted several times: ' + repeated.join(', '));
  });
});
...
test('③ several successive actions never re-emit a chunk that has already gone out', () => {
  isolated(() => {
    ...
    for (let geste = 1; geste <= 6; geste++) {
      const em = emission.emit({
        fresh: geste === 1 ? grosDoc() : [], budgetMax: 8000, nbFrames: 1, index: 1, scopeId: 'S',
      });
      for (const id of emittedIds(em.frames)) {
        if (vusGlobal.has(id)) duplicates.push(id);
        vusGlobal.add(id);
      }
    }
    assert.deepStrictEqual(duplicates, [], 'delivered twice: ' + duplicates.join(', '));
