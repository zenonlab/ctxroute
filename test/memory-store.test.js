...
const stateOf = (pairs) => {
  const e = createState();
  for (const [k, v] of pairs) poser(e, k, v);
  return e;
};
...
const WITHOUT_OUTPUT = { onExit: () => () => {} };
const freshEntry = (itemName) => createMemoryStore({ snapshotPath: path.join(TMP, itemName, 'snapshot.json'), ...WITHOUT_OUTPUT });
...
test('SAME CONTRACT as the disk store: absent = {}, written = read back identically', () => {
  const s = freshEntry('contrat');
  assert.deepEqual(s.loadState('doc-seen-', 'jamais-vu'), {},
    'an absent scope must answer {} — the fail-open the whole engine relies on');
  const state = { 'docs/a.md': { seen: true, sinceLastCall: 3 } };
  s.saveState('doc-seen-', 'sess', state);
  assert.deepEqual(s.loadState('doc-seen-', 'sess'), state);
});
...
test('A HOSTILE KEY IS DATA, never a prototype', () => {
  const s = freshEntry('proto');
  s.saveState('doc-seen-', '__proto__', { polluted: true });
  assert.deepEqual(s.loadState('doc-seen-', 'autre'), {},
    'another scope must stay empty — a Map cannot be polluted through a prototype');
  assert.deepEqual(s.loadState('doc-seen-', '__proto__'), { polluted: true });
});
...
test('LRU: READING a scope protects it from the next eviction', () => {
  ...
  s.saveState('doc-seen-', 'nouveau', { freshEntry: true });
...
test('SNAPSHOT: what a daemon knew, the next daemon finds again', () => {
  const filePath = path.join(TMP, 'reprise', 'snapshot.json');
...
test('SNAPSHOT: no temporary file survives the write', () => {
  const folder = path.join(TMP, 'restes');
  const s = createMemoryStore({ snapshotPath: path.join(folder, 'snapshot.json'), ...WITHOUT_OUTPUT });
  ...
  s.flush();
  assert.ok(fs.existsSync(path.join(folder, 'snapshot.json')), 'anti-vacuity: something must really have been written');
  assert.deepEqual(fs.readdirSync(folder).filter((f) => f.endsWith('.tmp')), [],
...
test('FAIL-OPEN: a CORRUPT snapshot yields an empty state, never an exception', () => {
  const filePath = path.join(TMP, 'corrompu', 'snapshot.json');
  fs.mkdirSync(path.dirname(filePath), { recursive: true });
  fs.writeFileSync(filePath, '{ this is not json');
  const s = createMemoryStore({ snapshotPath: filePath, ...WITHOUT_OUTPUT });
...
test('FAIL-OPEN: an ABSENT snapshot is a normal start, not a failure', () => {
  const s = createMemoryStore({ snapshotPath: path.join(TMP, 'jamais', 'snapshot.json'), ...WITHOUT_OUTPUT });
  assert.equal(s.restore(), 0);
});
...
test('FAIL-OPEN: a snapshot with the wrong SHAPE poisons nothing', () => {
  const filePath = path.join(TMP, 'forme', 'snapshot.json');
  fs.mkdirSync(path.dirname(filePath), { recursive: true });
  fs.writeFileSync(filePath, JSON.stringify({ pas: 'un tableau' }));
  assert.equal(createMemoryStore({ snapshotPath: filePath, ...WITHOUT_OUTPUT }).restore(), 0);

  fs.writeFileSync(filePath, JSON.stringify([['bonne-cle', { ok: 1 }], ['seule'], [42, {}], ['nul', null]]));
  const s = createMemoryStore({ snapshotPath: filePath, ...WITHOUT_OUTPUT });
...
test('PURGE: by PREFIX, and it counts what it really removed', () => {
  const m = stateOf([['doc-seen-a', {}], ['doc-seen-ab', {}], ['plan-a', {}], ['doc-seen-b', {}]]);
...
test('a flood of EPHEMERAL keys can never evict a DURABLE one', () => {
  ...
  poser(state, 'doc-seen-agent-A', { seen: ['doc'] });
  for (let i = 0; i < 50; i += 1) poser(state, `plan-agent-B--inv-${i}`, { segments: [] });

  evict(state, 4, 3);

  assert.ok(clefs(state).includes('doc-seen-agent-A'),
  ...
  assert.equal(clefs(state).filter((k) => k.startsWith('plan-')).length, 3,
...
test('each class evicts its OWN coldest, and the order is the LRU order', () => {
  const state = createState();
  for (const n of ['A', 'B', 'C']) poser(state, `doc-seen-${n}`, { seen: [] });
  for (const n of [1, 2, 3]) poser(state, `plan-x--inv-${n}`, { segments: [] });

  // A is the coldest durable, 1 the coldest plan — until A is READ.
  touch(state, 'doc-seen-A');
  evict(state, 2, 2);

  assert.ok(clefs(state).includes('doc-seen-A'), 'reading is a USE: the entry just read must not be the one dropped');
  assert.ok(!clefs(state).includes('doc-seen-B'), 'the coldest DURABLE goes, and it is B once A has been touched');
  assert.ok(!clefs(state).includes('plan-x--inv-1'), 'the coldest PLAN goes, independently of the durable class');
  assert.ok(clefs(state).includes('plan-x--inv-3'), 'and the youngest plan stays');
});
...
test('EVICT with no ephemeral ceiling given falls back to the DECLARED default', () => {
  ...
  for (let i = 0; i < MAX_EPHEMERAL + 1; i += 1) poser(full, `plan-y--inv-${i}`, {});
  assert.equal(evict(full, 0), 1,
...
test('COUNT + CLEAN EXIT: one snapshot per N mutations, carrying the CURRENT state', () => {
  const filePath = path.join(TMP, 'cadence', 'snapshot.json');
  ...
  let onOutput = null;
  const s = createMemoryStore({ snapshotPath: filePath, onExit: (fn) => { onOutput = fn; return () => { onOutput = null; }; } });
  ...
  assert.equal(fs.existsSync(filePath), true, 'the Nth mutation must flush');
  const written = new Map(JSON.parse(fs.readFileSync(filePath, 'utf8')));
  assert.deepEqual(written.get('doc-seen-sess'), { i: PERSIST_EVERY },
  ...
  assert.equal(fs.existsSync(filePath), false, 'the counter must RESET on a write, not drift');
  s.saveState('doc-seen-', 'sess', { tour2: PERSIST_EVERY });
  assert.equal(fs.existsSync(filePath), true, 'and the next full cycle writes again');
  ...
  assert.equal(typeof onOutput, 'function', 'a store with a snapshot path must REGISTER its exit hook, or the second authority does not exist');
  onOutput();
  assert.deepEqual(new Map(JSON.parse(fs.readFileSync(filePath, 'utf8'))).get('doc-seen-sess'), { final: true },
  ...
  assert.equal(fs.existsSync(filePath), false, 'and it must not have written anyway');

  s.close();
  assert.equal(onOutput, null, 'close() releases the listener — a store must not pile handlers onto a long-lived emitter');
});
