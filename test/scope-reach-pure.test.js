...
const entree = (nom) => `{"type":"tool_use","id":"x","name":"${nom}","input":{}}\n`;
...
test('ANCHOR — a tool call is counted, a skill or an agent name is NOT', () => {
  const texte = entree('Bash') + entree('mcp__ssh__ssh_exec')
    + '{"type":"skill","name":"ctxroute"}\n'
    + '{"type":"agent","name":"wz-design-engine"}\n';
  assert.deepStrictEqual(toolNamesFrom(texte).sort(), ['Bash', 'mcp__ssh__ssh_exec']);
});
...
const nomOutil = fc.stringMatching(/^[A-Za-z][A-Za-z0-9_-]{0,30}$/);
...
test('LAW round-trip — a transcript built from N names yields exactly those names', () => {
  fc.assert(fc.property(fc.uniqueArray(nomOutil, { maxLength: 12 }), (noms) => {
    assert.deepStrictEqual(toolNamesFrom(noms.map(entree).join('')).sort(), [...noms].sort());
  }));
});
...
test('LAW anchoring — a `name` OUTSIDE a tool-use entry never adds anything', () => {
  ...
  fc.assert(fc.property(fc.uniqueArray(nomOutil, { maxLength: 6 }), nomOutil, (vrais, intrus) => {
    fc.pre(!vrais.includes(intrus));
    const bruit = `{"type":"skill","name":"${intrus}"}\n{"type":"agent","name":"${intrus}"}\n`;
    assert.deepStrictEqual(toolNamesFrom(bruit + vrais.map(entree).join('') + bruit).sort(),
      [...vrais].sort());
  }));
});

test('LAW dedup — the same tool called N times appears ONCE', () => {
  fc.assert(fc.property(nomOutil, fc.integer({ min: 1, max: 20 }), (n, k) => {
...
test('LAW growth — appending text never REMOVES a name already found', () => {
  ...
  fc.assert(fc.property(fc.uniqueArray(nomOutil, { maxLength: 5 }), fc.string(), (noms, suite) => {
    const avant = toolNamesFrom(noms.map(entree).join(''));
    const apres = toolNamesFrom(noms.map(entree).join('') + suite);
    for (const n of avant) assert.ok(apres.includes(n), n);
  }));
});
