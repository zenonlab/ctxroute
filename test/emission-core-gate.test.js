...
const LAYER = 'src/emission-core.js';
...
function reachesTheLayer(rel, read) {
  const vus = new Set();
  const pile = [rel];
  while (pile.length > 0) {
    const cur = pile.pop();
...
test('GATE: every file that writes `additionalContext` goes through emission-core', () => {
  ...
  const offending = foundOnes.filter(
    (f) => !EXEMPTIONS[f] && !reachesTheLayer(f, lireReel)
  );
  assert.deepStrictEqual(
    offending,
    [],
    `These emitters compose their output WITHOUT going through ${LAYER}:\n  `
      + offending.join('\n  ')
...
test('NEGATIVE: an emitter deprived of the layer is DETECTED (gate not inert)', () => {
  ...
  const readSabotaged = (rel) => {
    const src = lireReel(rel);
    if (src === null) return null;
    return src.replace(/require\(\s*['"](?:\.\.?\/)+emission-core['"]\s*\)/g, 'null');
  };

  assert.ok(reachesTheLayer(target, lireReel), 'witness: intact, the target reaches the layer');
  assert.strictEqual(
    reachesTheLayer(target, readSabotaged),
    false,
