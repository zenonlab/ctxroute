...
ask(payload, { socketPath: adresse, frame: Number(frame), frames: Number(frames) },
  (r) => { process.send({ frame: Number(frame), text: JSON.stringify(r || null) }); });
`);
...
function startDaemon(address, snapshot) {
  return new Promise((pret, failure) => {
    const args = snapshot ? [address, snapshot] : [address];
    ...
    d.on('message', (m) => {
      ...
      if (typeof m === 'string' && m.startsWith('erreur:')) {
        failure(new Error(`the daemon could not take its address (${m.slice(7)}) at ${JSON.stringify(address)}`));
      }
    });
...
function knock(address, frame, frames) {
  return new Promise((res) => {
    const c = fork(CLIENT, [address, String(frame), String(frames)], { env: ENV, stdio: 'ignore' });
    c.on('message', (m) => res(m));
    c.on('exit', () => res({ frame, text: 'null' }));
  });
}
...
test('16 frame processes, ONE `once` document: delivered exactly once, with no lock and no file',
  { timeout: 30000 }, async () => {
    const address = testAddress('course');
    ...
    try {
      ...
      const responses = await Promise.all(
        Array.from({ length: 16 }, (_, i) => knock(address, i + 1, 16)),
      );

      const deliveries = responses.filter((r) => r.text.includes('CORPS-UNIQUE'));
      assert.equal(responses.length, 16, 'the sixteen clients must all have answered');
      assert.equal(deliveries.length, 1,
        `the document went out ${deliveries.length} time(s) instead of once. `
        + 'Frames that received it: ' + deliveries.map((r) => r.frame).join(',') + '. '
...
test('CONTROL: a fresh session really receives the document (the daemon is not mute)',
  { timeout: 30000 }, async () => {
    const address = testAddress('temoin');
    const daemon = await startDaemon(address);
    try {
      const r = await knock(address, 1, 1);
      assert.ok(r.text.includes('CORPS-UNIQUE'),
...
test('MEMORY IS CONSULTED: the second action on the same session is silent',
  { timeout: 30000 }, async () => {
    const address = testAddress('memoire');
    const daemon = await startDaemon(address);
    try {
      const un = await knock(address, 1, 1);
      const deux = await knock(address, 1, 1);
      assert.ok(un.text.includes('CORPS-UNIQUE'), 'first action delivers');
      assert.ok(!deux.text.includes('CORPS-UNIQUE'),
...
test('NO DAEMON: the client settles at once, silently, with no timer', { timeout: 30000 }, async () => {
  const address = testAddress('personne');   // nobody ever listened here
  const debut = Date.now();
  const r = await knock(address, 1, 1);
  const ms = Date.now() - debut;

  assert.equal(r.text, 'null', 'with no daemon the client must answer nothing, and never throw');
...
test('A RESTART DOES NOT RE-DELIVER: the state survives the death of the daemon',
  { timeout: 30000 }, async () => {
    const address = testAddress('reprise');
    const snapshot = path.join(TMP, 'reprise-snapshot.json');

    const un = await startDaemon(address, snapshot);
    const premier = await knock(address, 1, 1);
    ...
    const deux = await startDaemon(address, snapshot);
    const apres = await knock(address, 1, 1);
    const pid1 = un.pid;
    await stop(deux);

    assert.ok(premier.text.includes('CORPS-UNIQUE'), 'the first daemon must deliver — otherwise this cell proves nothing');
    assert.notEqual(deux.pid, pid1, 'the two daemons must really be different processes');
    assert.ok(!apres.text.includes('CORPS-UNIQUE'),
...
test('THE KERNEL SAYS WHO IS ALIVE — an address answers, or it is dead', { timeout: 30000 }, async () => {
  ...
  const daemon = await startDaemon(address);
  try {
    const vivant = await new Promise((r) => occupied(require('../src/kernel-endpoint.js').kernelAddress(address), r));
