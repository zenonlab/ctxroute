...
test('every refusal about the bound NAMES the key to write, literally', () => {
  const chemin = 'bounds.gateHookTimeoutSeconds';

  assert.throws(() => plan(manifest({ bounds: undefined }), machine()),
    (e) => e.message.includes(chemin), `a manifest with no \`bounds\` is refused without naming ${chemin}`);
  assert.throws(() => plan(manifest({ bounds: { gateHookTimeoutSeconds: 0 } }), machine()),
    (e) => e.message.includes(chemin), `a bound of 0 is refused without naming ${chemin}`);
  assert.throws(() => plan(manifest({
    consumers: [{ module: 'src/hooks/doc-inject.js', event: 'PreToolUse', matcher: '*', framed: true, timeout: 3 }],
  }), machine()), (e) => e.message.includes(chemin),
  `a framed consumer declaring its own timeout is refused without naming ${chemin} as the single place that number lives`);
});
