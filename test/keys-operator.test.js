...
import { matchingDocs, shouldSkip, composerKeys, composerKeysParAxe, keyDecision, inheritFilters } from '../src/sources/file.js';
...
const guard = (decl, cle) => keyDecision(decl, null).guard(cle);

const doc = (extra) => ({ doc: 'd', pattern: 'projet-x', ...extra });
const cmd = (command) => ({ toolName: 'Bash', toolInput: { command } });
const injected = (rule, payload) => matchingDocs([rule], payload).length === 1;
...
test('keys.match: CITING a project in a command no longer injects it…', () => {
  ...
  assert.ok(injected(doc({}), citation), 'PRECONDITION: without `keys` the citation injects (that is the defect)');
  assert.ok(!injected(doc({ keys: { match: ['-command'] } }), citation), 'blacklist: the command is out of the universe');
  assert.ok(!injected(doc({ keys: { match: ['file_path'] } }), citation), 'whitelist: only file_path remains, so no command');
});
...
test('…but WORKING in it still does — the half that matters', () => {
  ...
  for (const k of [undefined, { match: ['-command'] }, { match: ['file_path'] }]) {
    assert.ok(injected(doc(k ? { keys: k } : {}), travail), `real work must stay injected (keys=${JSON.stringify(k)})`);
  }
});
...
test('flat form = the SAME universe for the three axes (the shortcut)', () => {
  ...
  assert.ok(injected(doc({ keys: ['-command'] }), cmd('cd projet-x && ls')),
    'working IN the project must still inject: the designated directory is its own observable');
  assert.ok(!injected(doc({ keys: ['-command'] }), cmd('grep projet-x notes.md')),
  ...
  assert.ok(injected(doc({ pattern: 'projet-x/ls' }), cmd('cd projet-x && ls')),
  ...
  assert.ok(!injected(doc({ pattern: 'projet-x/ls', keys: ['-commandPaths'] }), cmd('cd projet-x && ls')),
    'the RECONSTRUCTED paths must be cuttable ALONE — that is what stops a citation from injecting');
  assert.ok(injected(doc({ pattern: 'projet-x/ls', keys: ['-commandCwd'] }), cmd('cd projet-x && ls')),
    'cutting the DIRECTORY must NOT cut the reconstruction: one name, one fact, and they are separable');
  assert.ok(injected(doc({ pattern: 'projet-x', keys: ['-command', '-commandPaths'] }), cmd('cd projet-x && ls')),
    'the pair an entry actually writes: WORKING in the project still injects (only the directory is read)');
  assert.ok(!injected(doc({ pattern: 'projet-x', keys: ['-command', '-commandPaths'] }), cmd('cd ailleurs && echo projet-x')),
    '…and CITING it fabricates nothing any more — the case that fired in production on 20/08');
  assert.ok(injected(doc({ keys: ['-content'] }), cmd('cd projet-x && ls')), 'removing an unrelated key changes nothing');
});
...
test('a WHITELIST REPLACES the universe — it can WIDEN, not only shrink', () => {
  ...
  assert.ok(!injected(doc({}), payload), 'PRECONDITION: unread by default');
  assert.ok(injected(doc({ keys: { match: ['description'] } }), payload), 'the entry widens its own universe');
});
...
test('⚠️ ANTI-INERTNESS: a rule WITHOUT `keys` is strictly unchanged', () => {
  ...
  assert.ok(injected(doc({}), { toolName: 'X', toolInput: input }));
...
test('an EMPTY or absent declaration never restricts (omission ≠ restriction)', () => {
  ...
  assert.ok(injected(doc({ keys: [] }), payload), 'an empty list is not a whitelist of nothing');
  assert.ok(injected(doc({ keys: { scope: ['-x'] } }), payload), 'an axis left out keeps its default universe');
});
...
test('the validator refuses the AMBIGUOUS, and only that', () => {
  ...
  for (const bad of [[], ['-'], [''], 'x', { dummy: ['a'] }, {}, { match: [] }]) {
...
test('the MIXED form ADJUSTS the default universe — minus the removals, plus the names', () => {
  ...
  assert.ok(injected(doc({}), payload), 'PRECONDITION: the raw command carries the pattern');
  assert.ok(!injected(doc({ keys: { match: ['-command'] } }), payload), 'the removal applies');
  assert.ok(injected(doc({ keys: { match: ['-command', 'description'] } }), payload),
  ...
  assert.ok(!injected(doc({ pattern: 'notes.md', keys: { match: ['-command', 'description'] } }), payload),
...
test('ADJUST keeps the SURVIVING default keys — an addition never replaces the rest', () => {
  ...
  assert.ok(injected(doc({ keys: { match: ['-remotePath', 'description'] } }), payload),
...
test('the ENTRY keeps the last word: its addition beats its category removal', () => {
  ...
  assert.ok(guard(compose, 'commandPaths'), 'the entry re-added the key its category had removed');
  assert.ok(!guard(compose, 'autre'), 'and its own removal still holds');
});
...
test('an entry naming NOTHING never narrows: the category stands (fail-open on garbage)', () => {
  ...
  for (const nothing of [[], 'x', 42, null]) {
    assert.deepStrictEqual(composerKeys(nothing, ['-command']), ['-command'], `entry ${JSON.stringify(nothing)}`);
  }
});
...
test('TOTALITY — a missing side is returned untouched, on both functions and both sides', () => {
  ...
  const alone = { match: ['-a'] };
  assert.strictEqual(composerKeysParAxe(undefined, alone), alone, 'no entry: the category object itself');
  assert.strictEqual(composerKeysParAxe(alone, undefined), alone, 'no category: the entry object itself');
...
test('heriterFiltres — the three filters, inherited or overridden, and NEVER a throw', () => {
  assert.deepStrictEqual(inheritFilters(undefined, undefined), { keys: undefined }, 'nothing in, nothing out');
  assert.deepStrictEqual(inheritFilters(null, null), { keys: undefined }, 'garbage in, still nothing out');
  assert.deepStrictEqual(inheritFilters({}, ['not', 'an', 'object']), { keys: undefined },
    'a `defaults` that is a LIST is inert — an engine that guesses must guess towards NOT injecting');
  assert.deepStrictEqual(inheritFilters({}, { scope: ['x'], exclude: ['y'] }), { keys: undefined, scope: ['x'], exclude: ['y'] },
    'an entry declaring nothing INHERITS its category — that is the whole point of the tier');
  assert.deepStrictEqual(inheritFilters({ scope: ['mien'], exclude: ['mien'] }, { scope: ['x'], exclude: ['y'] }),
    { keys: undefined, scope: ['mien'], exclude: ['mien'] }, 'the entry wins WHOLE: a filter is a VALUE, never a universe');
  assert.deepStrictEqual(inheritFilters({ scope: [], exclude: [] }, { scope: ['x'], exclude: ['y'] }),
    { keys: undefined, scope: ['x'], exclude: ['y'] }, 'an EMPTY list is "not declared" — historical loader semantics, kept to the byte');
  assert.deepStrictEqual(inheritFilters({ scope: ['a'] }, {}), { keys: undefined, scope: ['a'] },
...
test('heriterFiltres — the shapes a HAND-EDITED config really produces, none of them a throw', () => {
  ...
  assert.deepStrictEqual(inheritFilters('chaine', 'chaine'), { keys: undefined },
    'a string on both sides: inert, never a property read on a primitive');
  assert.deepStrictEqual(inheritFilters(['liste'], ['liste']), { keys: undefined },
  ...
  assert.deepStrictEqual(inheritFilters({ scope: [], exclude: [] }, { scope: [], exclude: [] }),
    { keys: undefined }, 'empty on both sides poses NOTHING');
  assert.deepStrictEqual(inheritFilters({ scope: [] }, { scope: ['x'] }), { keys: undefined, scope: ['x'] },
    'empty entry, non-empty category: the category decides');
  assert.deepStrictEqual(inheritFilters({ scope: ['a'] }, { scope: [] }), { keys: undefined, scope: ['a'] },
