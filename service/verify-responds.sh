...
#    unit it just installed (Linux: `ListenStream=` in the socket unit, the one
#    place the address lives there · macOS: `CTXROUTE_HTTP_PORT` in the plist)
#    or, on Windows where the Task Scheduler schema has NO element for it, from
#    the module's own `DEFAULT_PORT`. A number written again in this file would
#    be a second truth, and the day the unit moved its port this proof would
